...
FIRMWARENAME := $(ELBOX_MODEL_NAME)
FIRMWAREREV= $(shell echo $(ELBOX_FIRMWARE_VERSION) | cut -d. --output-delimiter=\"\" -f1,2)
BUILDNO :=$(shell cat buildno)
RELIMAGE:=$(shell echo $(FIRMWARENAME)_FW$(FIRMWAREREV)WW$(ELBOX_FIRMWARE_REVISION)_$(BUILDNO))
JOBS := -j$(shell grep -c processor /proc/cpuinfo)

ifeq ($(strip $(ELBOX_USE_IPV6)),y)
KERNELCONFIG := kernel.taurus.ipv6.config
else
KERNELCONFIG := kernel.taurus.config
endif
...
fakeroot_rootfs_image:
	...
	@./progs.board/template.taurus/makedevnodes rootfs
...
$(ROOTFS_IMG): ./tools/squashfs-tools-4.0_realtek/mksquashfs
	...
	$(Q)make clean_CVS
	$(Q)fakeroot make -f progs.board/template.taurus/config.mk fakeroot_rootfs_image
