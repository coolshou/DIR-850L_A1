...
int extract_addresses(HEADER *header, size_t qlen, char *name, time_t now, struct server *server)
#else
...
#ifdef ALPHA_DNS_HELPER_SUPPORT
			if ( daemon->o_helperscript != NULL )
			{
			  char cmd[128+ADDRSTRLEN*2];
			  char *t_addrbuff = safe_malloc(ADDRSTRLEN);
			  char *t_srv_addrbuff = safe_malloc(ADDRSTRLEN);
			  alpha_prettyprint_addr(addr, t_addrbuff, flags);
			  prettyprint_addr(&server->addr, t_srv_addrbuff);
			  snprintf(cmd, 127+ADDRSTRLEN*2, "sh %s get_addrs \"%s\" \"%s\" ", daemon->o_helperscript,
				t_addrbuff, t_srv_addrbuff );
			  // printf("ALPHA_DNS_DEBUG_INFO: get response addr:%s", cmd); /* print out to debug mode, use options: -d -q  */
			  extern int org_stdin,org_stdout, org_stderr;
			  alpha_system2(cmd, org_stdin, org_stdout, org_stderr);
			  free(t_addrbuff);
			  free(t_srv_addrbuff);
			}
#endif
