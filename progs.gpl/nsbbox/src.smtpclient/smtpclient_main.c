...
static char  *from_addr  = "someone@somewhere.com";
...
void usage(void)
{
    ...
    fprintf(stderr, "  -H, --src-host=HOST    host name to provide as source of message\n");
...
#ifdef NSBBOX
int smtpclient_main(int argc, char **argv)
#else
...
#endif
{
...
#else
    while ((c = getopt_long(argc, argv, ":H:s:f:r:S:P:Mvh", options, NULL)) != EOF) {
#endif
        switch (c) {
	    case 'H':
		src_host = optarg;
    ...
    if (hp->h_addrtype != AF_INET) {
        _log("unknown address family: %d", hp->h_addrtype);
        exit(1);
    }
