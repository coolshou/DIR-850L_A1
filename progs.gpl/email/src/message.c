...
//	char *subject=Mopts.subject;
	...
	if(sub_log_str != NULL)
	{
		...
		if ((fh = fopen(sub_log_str, "r")) != NULL)
		{
			int length;
			length = fread(text,1,512,fh);
			subject=text;
			fclose(fh);
		}
	}
	else{
			subject=Mopts.subject;
	}
