...
char *strrepl(char *orig, char * sfrom, char * sto)
{
    char * ret = NULL;
    char * tmp;
    char * ins;
    ...
    if ((orig == NULL) || (sfrom == NULL) || (sto == NULL))
        return ret;

    sfrom_len = strlen(sfrom);
    sto_len = strlen(sto);
    if (sfrom_len == 0)
        return ret;

    tmp = orig;
    for (count = 0; tmp = strstr(tmp, sfrom); count++)
    {
        tmp += sfrom_len;
    }

    tmp = ret = (char *)malloc(strlen(orig) + ((sto_len - sfrom_len) * count) + 1);
    if (ret == NULL)
        return ret;

    while (count--)
    {
        ins = strstr(orig, sfrom);
        ins_len = ins - orig;
        strncpy(tmp, orig, ins_len);
        tmp += ins_len;
        strncpy(tmp, sto, sto_len);
        tmp += sto_len;
        orig += ins_len + sfrom_len;
    }
    strcpy(tmp, orig);
...
int df_main(int argc, char **argv)
{
 ...
 set_error:
		...
		mount_point = strrepl(mount_entry->mnt_dir, "\\040", " ");
