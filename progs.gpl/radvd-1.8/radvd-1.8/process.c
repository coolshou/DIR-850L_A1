	...
	struct sockaddr_in6 *addr, struct in6_pktinfo *pkt_info, int hoplimit)
{
	...
	if (icmph->icmp6_type == ND_ROUTER_SOLICIT)
	{
		...
		dlog(LOG_DEBUG, 2, "received RS from %s", addr_str);//change to a debug level message 
		process_rs(iface, msg, len, addr);
	}
	else if (icmph->icmp6_type == ND_ROUTER_ADVERT)
	{
		//flog(LOG_WARNING, "received RA from %s", addr_str);
		dlog(LOG_DEBUG, 2, "received RA from %s", addr_str);//change to a debug level message
