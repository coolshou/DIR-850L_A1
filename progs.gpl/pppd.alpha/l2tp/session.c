...
l2tp_session * the_session = NULL;

static char * state_names[] = { "idle", "wait-tunnel", "wait-reply", "wait-connect", "established" };
...
void session_send_sli(l2tp_session * ses)
{
	...
	dgram = l2tp_dgram_new_control(MESSAGE_SLI, ses->tunnel->assigned_id, ses->assigned_id);
	if (!dgram) return;

	/* Add ACCM */
	l2tp_dgram_add_avp(dgram, ses->tunnel, MANDATORY, sizeof(buf), VENDOR_IETF, AVP_ACCM, buf);
 ...
 * DESCRIPTION:
 *   Initiates session setup. Once call is active, established() will be called.
 ***************************************************************************/
l2tp_session * l2tp_session_call_lns(l2tp_peer * peer, char const * calling_number, void * private)
{
	...
	if (!ses)
	{
		d_error("l2tp: session_call_lns(): out of memory");
		return NULL;
	}

	/* Establish a tunnel to the peer */
	tunnel = l2tp_tunnel_establish(peer);
	if (!tunnel)
	{
		free(ses);
		return NULL;
	...
	if (l2tp_sync)
	{
		ses->handle_frame_from_tunnel = l2tp_sync_ppp_handle_frame_from_tunnel;
		ses->handle_frame_to_tunnel   = l2tp_sync_ppp_handle_frame_to_tunnel;
	}
	else
	{
		ses->handle_frame_from_tunnel = l2tp_async_ppp_handle_frame_from_tunnel;
		ses->handle_frame_to_tunnel   = l2tp_async_ppp_handle_frame_to_tunnel;
	}
...
void l2tp_session_send_CDN(l2tp_session * ses, int result_code, int error_code, char const * fmt, ...)
{
	...
	buf[0] = result_code / 0x100;
	buf[1] = result_code & 0xff;
	buf[2] = error_code / 0x100;
...
void l2tp_session_lcp_snoop(l2tp_session * ses, unsigned char const * buf, int len, int incoming)
{
	...
	if (ses->we_are_lac)
	{
		d_dbg("l2tp: Turning off snooping: We are LAC.\n");
		ses->snooping = 0;
		return;
	}
	...
	if (protocol <= 0x3fff)
	{
		d_dbg("l2tp: Tunring off snooping: Network protocol %04x found.\n", protocol);
		ses->snooping = 0;
		return;
	}

	/* If it's not LCP, do not snoop */
	if (protocol != 0xc021) return;
	...
	while (len > 0)
	{
		...
		len -= opt_len;
		buf += opt_len;
		d_dbg("l2tp: Found option type %02x; len %d\n", opt, opt_len);
		...
		if (opt == 0x02 && opt_len == 0x06)
		{
			...
			{	/* ACCM negotiation REJECTED; use default */
				accm = 0xffffffff;
				d_dbg("l2tp: Reject ACCM negotiation; defaulting (%s)\n", incoming ? "incoming" : "outgoing");
				/* ??? Is this right? */
				ses->recv_accm = accm;
				ses->send_accm = accm;
				ses->got_recv_accm = 1;
			...
			{
				d_dbg("l2tp: Sending SLI: Send ACCM = %08x; Receive ACCM = %08x\n", ses->send_accm, ses->recv_accm);
...
void l2tp_session_handle_ICRP(l2tp_session * ses, l2tp_dgram * dgram)
{
	...
	if (!val)
	{
		d_error("l2tp: No assigned session-ID in ICRP\n");
		return;
	}
	if (!l2tp_dgram_validate_avp(VENDOR_IETF, AVP_ASSIGNED_SESSION_ID, len, mandatory))
	{
		d_error("l2tp: Invalid assigned session-ID in ICRP\n");
		return;
	}
...
void l2tp_session_handle_ICCN(l2tp_session * ses, l2tp_dgram * dgram)
{
	...
	{
		l2tp_session_send_CDN(ses, RESULT_FSM_ERROR, 0, "Received ICCN for session in state %s", state_names[ses->state]);
		return;
	}
	...
	while (1)
	{
		...
		if (!val)
		{
			if (err)
			{
				l2tp_session_send_CDN(ses, RESULT_GENERAL_ERROR, ERROR_BAD_VALUE, "%s", l2tp_get_errmsg());
				return;
			}
			break;
		}
		if (vendor != VENDOR_IETF)
		{
			...
			l2tp_session_send_CDN(ses, RESULT_GENERAL_ERROR, ERROR_UNKNOWN_AVP_WITH_M_BIT,
