...
{
	int addr_type;
	struct universal_sockaddr addr;		/* Peer's address (ipv4) */
...
typedef struct l2tp_tunnel_t
{
	...
	struct universal_sockaddr peer_addr;		/* Peer's address */
...
void l2tp_tunnel_handle_received_control_datagram(l2tp_dgram * dgram, struct universal_sockaddr *from);
