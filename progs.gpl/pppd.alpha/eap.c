...
#define RCSID	"$Id: eap.c,v 1.1.1.1 2005/05/19 10:53:06 r01122 Exp $"
...
static void eap_server_timeout(void * eloop_ctx, void *timeout_ctx);
...
int unit;
{
	...
	esp->es_server.ea_id = (u_char)(lrand48() & 0xff);
...
eap_state *esp;
{
	...
	case eapMD5Chall:
		...
		challen = (lrand48() % (MAX_CHALLENGE_LENGTH - MIN_CHALLENGE_LENGTH)) + MIN_CHALLENGE_LENGTH;
		PUTCHAR(challen, outp);
		esp->es_challen = challen;
		ptr = esp->es_challenge;
		while (--challen >= 0)
			//*ptr++ = (u_char) (drand48() * 0x100);
			*ptr++ = (u_char)(lrand48() & 0xff);
	...
	case eapSRP4:
		...
		challen = (lrand48() % (MAX_CHALLENGE_LEN - MIN_CHALLENGE_LENGTH)) + MIN_CHALLENGE_LENGTH;
		esp->es_challen = challen;
		ptr = esp->es_challenge;
		while (--challen >= 0)
			//*ptr++ = drand48() * 0x100;
...
static void eap_server_timeout(void * eloop_ctx, void * timeout_ctx)
{
	eap_state *esp = (eap_state *)timeout_ctx;

	if (!eap_server_active(esp)) return;
