 ...
 *
 * $Id: pptp_ctrl.h,v 1.1.1.1 2005/05/19 10:53:06 r01122 Exp $
 */
...
#ifdef ELBOX_PROGS_GPL_PPPD_ACCEL_PPTP_PLUS
PPTP_CALL * pptp_call_open(PPTP_CONN * conn, int call_id, pptp_call_cb callback, char *phonenr,int window);
int pptp_conn_established(PPTP_CONN * conn);
#else
PPTP_CALL *pptp_call_open(PPTP_CONN * conn, int call_id, pptp_call_cb callback, char *phonenr);
#endif
