        ...
        char *phonenr,int window)
{
    ...
    call->closure   = NULL;
    packet.call_id = hton16(call->call_id);
    packet.call_sernum = hton16(call->sernum);
...
{
int iii=0;
unsigned char *ppp=buffer;
for(iii=0; iii< size; iii++){
	if(iii%16 == 0)
		printf("\n");
	printf("%02x ", ppp[iii]); 
}
printf("\n total size=%d, write to socket %d\n", size, conn->inet_sock);
}
#endif
...
int ctrlp_disp(PPTP_CONN * conn, void * buffer, size_t size)
{
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_OUT_CALL_RPLY:
        {
            ...
            log("Received Outgoing Call Reply. callid= %i", callid);
