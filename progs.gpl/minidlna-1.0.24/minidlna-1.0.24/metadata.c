...
GetAudioMetadata(const char * path, char * name)
{
	...
	else if( ends_with(path, ".3gp") )
	{
		...
		//+++ Jerry Kao, Modified for [7.3.28.1.6].
		#ifdef DLNA_1_5	
			m.mime = strdup("audio/mp4");
		#else
...
#ifdef DLNA_1_5
	//+++ Jerry kao, add to skip following MediaTypes for DLNA basic test.
	if (!strcmp(m.dlna_pn, "AAC_ISO_320")) {
		printf(" +++ [%s]_Not Insert [Audio]: %s\n", __func__, m.dlna_pn);		
	}
	else
	{				
#endif			
...
#ifdef DLNA_1_5
	//+++ Jerry kao, add to skip MediaTypes: JPEG_LRG and JPEG_MED for DLNA basic test.					
	if (!strcmp(m.dlna_pn, "JPEG_LRG") || !strcmp(m.dlna_pn, "JPEG_MED")) {
		printf(" +++ [%s]_Not Insert [Image]: %s\n", __func__, m.dlna_pn);		
	}
	else
	{				
#endif	
...
GetVideoMetadata(const char * path, char * name)
{
	...
	DPRINTF(E_DEBUG, L_METADATA, " ====== path: [%s], name: [%s]\n", path, name);	

//#if 1	//+++ Jerry Kao, marked due to cannot open Video using FFmpeg lib.
	av_register_all();
	
	#if LIBAVFORMAT_VERSION_INT >= ((53<<16)+(2<<8)+0)
	if( avformat_open_input(&ctx, path, NULL, NULL) != 0 )		// run here
	#else
...
//#endif 	//#if 1	
	...
	//+++ Jerry kao, classify files into Video type according to EXT name.
	/*
	if( ends_with(path, ".mpg") )
	{		
		asprintf(&m.mime, "video/mpeg");
	}
