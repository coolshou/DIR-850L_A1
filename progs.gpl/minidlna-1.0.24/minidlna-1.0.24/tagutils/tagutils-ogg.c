...
{
	//ogg_stream_pagein(&stream->os, page);  
	//because of unknow type. mean DUT may not support this codec. dont waste resouce to parsing
	//jef 2013/05/13
	//ref: https://bugzilla.redhat.com/show_bug.cgi?format=multiple&id=223988 comment5
}
		   ...
		   ogg_int64_t *written)
{
	...
	while((ret = ogg_sync_pageout(sync, page)) <= 0)
	{
		...
		if(buffer == NULL)
        { DPRINTF(E_WARN, L_SCANNER, "######### Corrupted ogg.\n"); return 0;}
