...
/* 
//+++ Jerry Kao, added to find height from Sequence Header (prefix = 0x1b3) 
      in order to distinguish PAL and NTSC format by the height 
      of frame for DLNA Logo test. 

      Function: mpegps_read_height_from_seq_header
      Return: the value of height of frame.
*/
static int mpegps_read_height_from_seq_header(AVFormatContext *s)
{		  
    AVIOContext *tmp_pb = s->pb;    
    int32_t tmp_m;
    int size, startcode_seq, mpeg_height = 0; 	 	 	 	  
     	 	 	
redo_seq:		  	    	    	    	    	    	    
    tmp_m = 0xff;
    size = MAX_SYNC_SIZE;

    startcode_seq = find_next_start_code(tmp_pb, &size, &tmp_m);	     
    ...
    if (startcode_seq != 0x1b3)
    	goto redo_seq;    	    	    		    	           	    	    
    
    avio_r8(tmp_pb);
    
    mpeg_height = (avio_r8(tmp_pb) << 8) | avio_r8(tmp_pb);	    	    	      
   	   	
    //printf("    ++++[%s]== %d === mpeg_height= %d \n", __func__, __LINE__, mpeg_height);
                                  ...
                                  int64_t *ppts, int64_t *pdts)
{
 ...
 redo:
    ...
    //printf("    ++++[%s]== %d === Found startcode = %x \n", __func__, __LINE__, startcode);
                              ...
                              AVPacket *pkt)
{
 ...
 found:
    ...
    //+++ Jerry Kao, added to read height from Sequence Header (in order to distinguish PAL/NTSC format). 
    if (type == AVMEDIA_TYPE_VIDEO)
    {    	    	    	    					
    	st->codec->height = mpegps_read_height_from_seq_header(s);  	
    	//printf("    ++++[%s]== %d === height = %d \n", __func__, __LINE__, st->codec->height);  
    }
