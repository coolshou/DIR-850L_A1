 ...
 * Forwards
 */
static int scan_path(char *path, char *code_page);
static int scan_get_info(char *file, MP3FILE *pmp3);
static int scan_freetags(MP3FILE *pmp3);
static void scan_music_file(char *path, char *fname,struct stat *psb, int is_compdir, char *code_page);

static TAGHANDLER *scan_gethandler(char *type);
static int mp3_counter;
...
void resize_id3_tag(MP3FILE *mp3file){
	int tag_length = 0;
	
	if(mp3file->fname)
	tag_length = strlen(mp3file->fname);
	...
	tag_length = 0;
	if(mp3file->title)
		tag_length = strlen(mp3file->title);
	if(tag_length > 255)
		*(mp3file->title+255)=0;
	
	tag_length = 0;
	if(mp3file->artist)
		tag_length = strlen(mp3file->artist);
	if(tag_length > 255)
		*(mp3file->artist+255)=0;
		
	tag_length = 0;
	if(mp3file->album)
		tag_length = strlen(mp3file->album);
	if(tag_length > 255)
		*(mp3file->album+255)=0;
		
	tag_length = 0;
	if(mp3file->genre)
		tag_length = strlen(mp3file->genre);
	if(tag_length > 255)
		*(mp3file->genre+255)=0;
	
	tag_length = 0;
	if(mp3file->type)
		tag_length = strlen(mp3file->type);
	if(tag_length > 255)
		*(mp3file->type+255)=0;

	tag_length = 0;
	if(mp3file->grouping)	
		tag_length = strlen(mp3file->grouping);
	if(tag_length > 255)
		*(mp3file->grouping+255)=0;

	tag_length = 0;		
	if(mp3file->comment)	
		tag_length = strlen(mp3file->comment);
	if(tag_length > 255)
		*(mp3file->comment+255)=0;
}

int charset_convertor(char *fromcode, MP3FILE *mp3file)
{
		char	*title_result = NULL;
		char	*artist_result = NULL;
		char	*album_result = NULL;
		char	*genre_result = NULL;
 		
		/* convert song title */
		if( mp3file->title != NULL ) {
			DBG_FILE_SCAN(printf("mp3-scanner.c: charset_convertor(): convert song title\n");)
			if( iconv_string("UTF-8",fromcode, mp3file->title, mp3file->title+strlen(mp3file->title)+1, 
			&title_result, NULL) < 0 ) {
				DBG_FILE_SCAN(printf("---- title iconv_string failed ----\n");)
			}
			else {
				mp3file->title = title_result;
			}
		}
		/* convert song artist */
		if( mp3file->artist != NULL )	{
			DBG_FILE_SCAN(printf("mp3-scanner.c: charset_convertor(): convert song artist\n");)
			if( iconv_string("UTF-8",fromcode, mp3file->artist, mp3file->artist+strlen(mp3file->artist)+1, 
			&artist_result, NULL) < 0 ) {
				DBG_FILE_SCAN(printf("---- artist iconv_string failed ----\n");)
			}
			else {
				mp3file->artist=artist_result;
			}
		}
		/* convert song album */
		if ( mp3file->album != NULL ) {
			DBG_FILE_SCAN(printf("mp3-scanner.c: charset_convertor(): convert song album\n");)
			if( iconv_string("UTF-8",fromcode, mp3file->album, mp3file->album+strlen(mp3file->album)+1, 
			&album_result, NULL) < 0 ) {
				DBG_FILE_SCAN(printf("---- album iconv_string failed ----\n");)
			}
			else {
				mp3file->album=album_result;
			}		
		}
		/* convert song genre */
		if( mp3file->genre != NULL ) {
			DBG_FILE_SCAN(printf("mp3-scanner.c: charset_convertor(): convert song genre\n");)
			if( iconv_string("UTF-8",fromcode, mp3file->genre, mp3file->genre+strlen(mp3file->genre)+1, 
			&genre_result, NULL) < 0 ) {
				DBG_FILE_SCAN(printf("---- genre iconv_string failed ----\n");)
			}
			else {
				mp3file->genre=genre_result;						
			}
		}
...
int scan_init(char **patharray,char *code_page) {
    int err=0;
    int index=0;
    char resolved_path[PATH_MAX];
		...
		mp3_counter=0;
		//LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_COUNTER,ITUNES_OK,MIPC_TIME_ALWAYS,(int)mp3_counter);
    ...
    conversion_codepage = conf_alloc_string("scanning","mp3_tag_codepage",
                                            code_page);   
    printf("mp3-scanner.c: scan-init: conversion_codepage=%s\n",conversion_codepage);
    
		/* curtis@alpha add strlen(patharray[index] to skip scan root folder 06_30_2009 */
    while((patharray[index] != NULL) && (!util_must_exit())) { 
    	  
    	  if (strlen(patharray[index]) >4 ){// /mnt/HD/ 			
	        DPRINTF(E_DBG,L_SCAN,"Scanning for MP3s in %s\n",patharray[index]);
	        if (realpath(patharray[index],resolved_path) == NULL)
			{
				DPRINTF(E_INF,L_SCAN,"%s realpath error: %s\n", __FUNCTION__, strerror(errno));
				index++;
				continue;
			}
	        err=scan_path(resolved_path, conversion_codepage);
      	}
      	else
      		printf("mp3-scanner.c: scan_init(): error path=%s\n",patharray[index]);
 ...
 */
int scan_path(char *path, char *code_page) {
    DIR *current_dir;
    ...
    while(1) {
        ...
        if(!os_lstat(relative_path,&sb)) {
           	...
           	if(S_ISLNK(sb.st_mode) && !follow_symlinks && dirlevel!=0)
                continue;
        }

        mp3_path[0] = '\x0';
        if (realpath(relative_path,mp3_path) == NULL)
		{
			DPRINTF(E_INF,L_SCAN,"%s realpath error: %s\n", __FUNCTION__, strerror(errno));
			continue;
		}
            ...
            if(S_ISDIR(sb.st_mode)) {  /* follow dir */
                if(conf_get_int("scanning","ignore_appledouble",1) &&
                   ((strcasecmp(pde->d_name,".AppleDouble") == 0) ||
                    ...
                    scan_path(mp3_path, code_page);
               }
            } else {
                scan_filename(mp3_path, is_compdir, extensions, code_page);
            }
        }
    }
	dirlevel--;
 ...
 *        mp3-scanner.c
 */
void scan_filename(char *path, int compdir, char *extensions, char *code_page) {
    ...
    if (realpath(path,mp3_path) == NULL)
	{
		DPRINTF(E_INF,L_SCAN,"%s realpath error: %s\n", __FUNCTION__, strerror(errno));
		return;
	}
            ...
            if(ext && ((int)strlen(ext) > 1)) {
                if(strcasecmp(".m3u",ext) == 0) {
                    ...
                    if((!pmp3) || (pmp3->db_timestamp < mod_time) ||
                       (pmp3->force_update)) {
                        scan_music_file(path,fname,&sb,is_compdir, code_page);
                    } else {
...
void scan_music_file(char *path, char *fname,
                     struct stat *psb, int is_compdir, char *code_page) {
    MP3FILE mp3file;
    char *current=NULL;
    char *type;
    ...
    if(scan_get_info(mp3file.path,&mp3file)) {
        if(is_compdir)
    		...
    		/* detect charset and transform it to UTF-8 curtis@alpha 03_23_2009 
    		 * modify input parameter from MP3FILE to string curtis@alpha 03_30_2009
    		 */
    		 
				if (!unicode16_text)
    			charset_convertor(code_page, &mp3file);
        ...
        */
        ...
        resize_id3_tag(&mp3file);
        db_add(NULL,&mp3file,NULL);
        mp3_counter++; 
        /* set value into IPC server */
		//LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_COUNTER,ITUNES_OK,MIPC_TIME_ALWAYS,(int)mp3_counter);    
    } else {
