...
int u_open(u_port_t port) {
   ...
   //+++sam_pan
   if(strcmp(device_inf, "Any"))
   {										
		struct ifreq ifr;
		
		ifr.ifr_addr.sa_family = AF_INET;
		strncpy(ifr.ifr_name, device_inf, IFNAMSIZ-1);
		if(ioctl(sock, SIOCGIFADDR, &ifr)==-1) return -1;
		server.sin_addr = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
		DPRINTF(E_LOG,L_WS,"iTunes server listen at interface = %s:%s\n", device_inf, inet_ntoa(server.sin_addr));			
   }
   else	
   {	   		
		server.sin_addr.s_addr = htonl(INADDR_ANY);   		
		DPRINTF(E_LOG,L_WS,"iTunes server listen at any address");
   }	
   //---sam_pan
