...
/* Bouble - 20100507 
  ...
  *
  * But, base on ALPHA design, all packets from WAN will be dropped by default(at NAT prerouting chain), 
  * if doesn't have explicit rule to accept it.
  *
  * Base on below code, it will inhibited "DROP" target at NAT table.
  * It means that our rules will not take effect.
  *
  * Finally, obey ALPHA design, and disable below code
  */
#if 0
