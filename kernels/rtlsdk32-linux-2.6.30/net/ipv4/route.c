...
//Alpha
#ifdef CONFIG_HARDWARE_TURBO
static int __mkroute_input_6(struct sk_buff *skb,
			   struct fib_result* res,
			   struct in_device *in_dev,
...
//Alpha
#ifdef CONFIG_HARDWARE_TURBO
static int ip_mkroute_input_6(struct sk_buff *skb,
				struct fib_result* res,
...
//Alpha
#ifdef CONFIG_HARDWARE_TURBO
				.iif = lsrc? dev_net(dev)->loopback_dev->ifindex : dev->ifindex,
				.lsrc = lsrc,
				.gw = 0
#else
			    .iif = dev->ifindex 
...
//Alpha
	...
	if (lsrc)
	{
		...
		if (ipv4_is_multicast(lsrc) || IN_BADCLASS(lsrc) || ipv4_is_zeronet(lsrc) || ipv4_is_loopback(lsrc))
			goto e_inval;
		/* It is equivalent to inet_addr_type(lsrc) == RTN_LOCAL */
		dev_out = ip_dev_find(net , lsrc);
...
static void rt_balance_rate_calculator(unsigned long dummy)
{
	...
	for (dp=&dev_base; (d=*dp) != NULL; dp=&d->next)
	{
		...
		if (dev_stat)
		{
			...
			{
				ev_rate = (dev_stat->rx_bytes/2 + dev_stat->tx_bytes/2 - dev_stat->lbytes/2);
			}
#if 0
