...
//Alpha
unsigned int _lan_ip=0;//0xC0A80001;
...
int isUsbIp_Reserved(struct sk_buff *skb, unsigned int hooknum, int direction)
{
	...
	if(iph->protocol ==IPPROTO_TCP){
		...
		if(direction==0){ //rx we check dest port
			if(th->dest==USBIP_PORT_NUM && skb->pkt_type== PACKET_HOST)
				ret=1;
			if(th->dest==USBIP_PORT_NUM_1 && skb->pkt_type== PACKET_HOST)
				ret=1;
			if(th->dest==UWIFI_PORT_NUM && skb->pkt_type== PACKET_HOST)
				ret=1;	

		}
		if(direction==1){ //tx we check src port
			if(th->source==USBIP_PORT_NUM && skb->pkt_type== PACKET_HOST)
				ret=1;
			if(th->source==USBIP_PORT_NUM_1 && skb->pkt_type== PACKET_HOST)
				ret=1;
			if(th->source==UWIFI_PORT_NUM && skb->pkt_type== PACKET_HOST)
				ret=1;	

		}
#else
		if(direction==0){ //rx we check dest port
			if((th->dest==USBIP_PORT_NUM || th->dest==HTTP_PORT_NUM) && skb->pkt_type== PACKET_HOST)
				ret=1;
			if((th->dest==USBIP_PORT_NUM_1 || th->dest==UWIFI_PORT_NUM) && skb->pkt_type== PACKET_HOST)
				ret=1;
		}
		if(direction==1){ //tx we check src port
			if((th->source==USBIP_PORT_NUM || th->source==HTTP_PORT_NUM) && skb->pkt_type== PACKET_HOST)
				ret=1;
			if((th->source==USBIP_PORT_NUM_1 || th->source==UWIFI_PORT_NUM) && skb->pkt_type== PACKET_HOST)
				ret=1;	
		}
#endif
		if(!ret)
