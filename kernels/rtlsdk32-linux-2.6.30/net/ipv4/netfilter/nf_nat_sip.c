		    ...
		    union nf_inet_addr *addr, __be16 port)
{
	...
	__be16 newport;
//Alpha
//marco, mod. the map_addr to pass cd-router test
//before,linux kernel will only magle packet if ip and port match
//but it will have problem if the packet source port is different from
//the one in Via header
#if 1
			       ...
			       const char **dptr, unsigned int *datalen)
{
	...
	if (ct_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
		...
		char buffer[sizeof("nnn.nnn.nnn.nnn:nnnnn")];
//Alpha
//marco, mod. below part to pass cd-router test
//before,linux kernel will only magle packet if ip and port match
//but it will have problem if the packet source port is different from
//the one in Via header
#if 0
...
//Alpha
//marco, support outbound calls with multiple SIP clients
#if 1
	if(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port ==
		exp->tuple.dst.u.udp.port)
	{
		range.min.all=exp->saved_proto.udp.port;
		range.max.all=exp->saved_proto.udp.port;
	}
#endif	
...
TRY_AGAIN:
	if(try_count>0)
	{
		rtp_exp->tuple.dst.u.udp.port+=htons(1000); 
		rtcp_exp->tuple.dst.u.udp.port+=htons(1000); 
	}
#endif	
...
//marco, support outbound calls with multiple SIP clients
	...
	if (port == 0)
	{
		...
		{
			printk("The port is in used, try another one\n");
			try_count++;
			goto TRY_AGAIN;
		}	
	}
