...
static int find_appropriate_dest(struct net *net,
							const struct nf_conntrack_tuple *tuple,
							const struct nf_conntrack_tuple *orig_tuple, int lock)
{
	unsigned int h = hash_by_src(tuple);
...
TRY_ANOTHER_PORT:
			...
			if ( find_appropriate_dest(net, tuple,orig_tuple,0) )
...
#endif
	{
		if (nat->bydest.pprev != NULL) hlist_del_rcu(&nat->bydest);
	}
#endif
	spin_unlock_bh(&nf_nat_lock);
}
