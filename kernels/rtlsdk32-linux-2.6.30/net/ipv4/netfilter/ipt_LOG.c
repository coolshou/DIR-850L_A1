...
//Alpha		
		//+++ hendry add for access control web access log, copy from kwest.
		#if 1 
				...
				if (match_http_method(line) && match_http_version(line))
				{
					...
					{												
						...
						while (datalen > 6) /* 6 characters: "Host: " */
						{
							...
							datalen -= line_len;
							start = end;							
							//printk(KERN_DEBUG "\nline in while loop--------->%s \n\n", line);
							//printk(KERN_DEBUG "\nline len in while ------>%d\n", line_len);					
							//printk(KERN_DEBUG "\ntotal data len - line len in while------> %d\n", datalen);
