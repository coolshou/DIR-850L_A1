...
//Alpha
void set_tcp_timeout_established(int timeout)
{
	tcp_timeouts[TCP_CONNTRACK_ESTABLISHED]=timeout*HZ;
}
EXPORT_SYMBOL(set_tcp_timeout_established);

//Alpha
#ifdef CONFIG_SOFTWARE_TURBO
unsigned int get_tcp_timeout_established(void)
{
	return tcp_timeouts[TCP_CONNTRACK_ESTABLISHED];
}

EXPORT_SYMBOL(get_tcp_timeout_established);
#endif
...
//Alpha
	...
	if(hw_tcpip && hw_tcpip->refresh_tcp_connection)
		hw_tcpip->refresh_tcp_connection(th, ct);
#endif
...
//Alpha
#if defined(FAST_PATH_SPI_ENABLED) || !(defined(CONFIG_RTL_IPTABLES_FAST_PATH) ||defined(CONFIG_RTL_HARDWARE_NAT)) || defined(CONFIG_HARDWARE_TURBO)
	if (
#if defined(FAST_PATH_SPI_ENABLED)
(fast_spi == 1) &&
#endif
