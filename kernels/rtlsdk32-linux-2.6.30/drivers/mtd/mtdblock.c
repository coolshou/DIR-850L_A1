...
#else
int flash_hw_start=0x10000;	// hw setting start address
int flash_hw_len=0x10000;	// hw setting length
int flash_devdata_start=0x20000;
int flash_devdata_len=0x10000;
int flash_devconf_start=0x30000;
int flash_devconf_len=0x10000;
#endif /* 0, Alpha. */
int flash_write_flag=0;		// 1: hw setting 2: default setting, 4: current setting, 8: system image
#endif
			    ...
			    int len, const char *buf)
{
	...
	DEBUG(MTD_DEBUG_LEVEL2, "mtdblock: write on \"%s\" at 0x%lx, size 0x%x\n",
		mtd->name, pos, len);
#if 0
// 1: hw setting 2: default setting, 4: current setting, 8: system image
...
#endif
// dennis change
// 1: hw setting 2: devdata setting, 4: devconf setting, 8: system image
#ifdef CONFIG_RTL_819X
#ifdef CONFIG_RTL_FLASH_MAPPING_ENABLE
	/*since len is normal 0x200 less than every section*/
	if(flash_write_flag != 0x8000) 
	{
		flash_write_flag = 0;
		if ( pos >= CONFIG_RTL_HW_SETTING_OFFSET && pos < CONFIG_RTL_DEVDATA_OFFSET ) {
			flash_write_flag |= 1;
			if ((pos+len) > CONFIG_RTL_DEVDATA_OFFSET ) {
				flash_write_flag |= 2;
				if ((pos+len) > CONFIG_RTL_DEVCONF_OFFSET )
					flash_write_flag |= 4;
			}
		}
		if ( pos >= CONFIG_RTL_DEVDATA_OFFSET && pos < CONFIG_RTL_DEVCONF_OFFSET ) {
			flash_write_flag |= 2;
			if ((pos+len) > CONFIG_RTL_DEVCONF_OFFSET ) {
				flash_write_flag |= 4;
			}
		}
		else if ( pos >= CONFIG_RTL_DEVCONF_OFFSET && pos < CONFIG_RTL_RETAIN_OFFSET ){
			flash_write_flag |= 4;
		}
	}
#else
