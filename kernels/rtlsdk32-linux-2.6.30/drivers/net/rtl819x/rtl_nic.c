...
/*
 * the loop at interrupt_dsr_rx will cause LAN client can not
 * access web UI at flooding attack. so set loop count to 32
 */
#define ALPHA_DOS_ATTACK 1
...
//Enable jumbo frame
			REG32(PCRP0+(port<<2))= ((REG32(PCRP0+(port<<2)))|(AcptMaxLen_9K));
			TOGGLE_BIT_IN_REG_TWICE(PCRP0+(port<<2),AcptMaxLen_9K);
			TOGGLE_BIT_IN_REG_TWICE(PCRP0+(port<<2),EnablePHYIf);
			TOGGLE_BIT_IN_REG_TWICE(PCRP0+(port<<2),ForceLink);
#endif
#else
...
//Alpha
	extern int alpha_port_phyAbility_set(uint32 port, uint32 Speed,uint32 Duplex);
#endif
...
#endif
//Alpha
//polo add for 10M/100M setting.
		case RTL8651_IOCTL_SETWANLINKSPEED:
			{
#ifdef CONFIG_RTL_8367R_SUPPORT
				pRet = (int32 *)args[3];
				if (*pRet == 0x08) /* 10M only */
				{
					speed = 10;
					duplex = 1;
				}
				else if (*pRet == 0x09) /* 100M only */	
				{
					speed = 100;
					duplex = 1;
				}
				else if(*pRet == 0x0a) /* 1000M only */
				{
					speed = 1000;
					duplex = 1;
				}
				...
				if(alpha_port_phyAbility_set(RTL8367R_WAN,speed,duplex) == 0)
					rc = SUCCESS;
				break;
#else
				/* 
				 ...
				 */		
				//1.get wan port value.
				
				int i;
				int wanPortMask;
				int32 totalVlans;
				uint32 phyId,statCtrlReg;
...
{
	char 	tmpbuf[64];
	uint32	port, mode, phy_mode, tx_time, tx_cnt, interval = DEF_PKT_INTERVAL;
	int 		i, j;
