...
//HAPD&REPEATOR 2012.09.07
int psk_indicate_evt_hapd(struct rtl8192cd_priv *priv, int id, unsigned char *mac, unsigned char *msg, int len)
{
	struct stat_info *pstat;
...
int psk_indicate_evt(struct rtl8192cd_priv *priv, int id, unsigned char *mac, unsigned char *msg, int len)
{
	...
	if(OPMODE & WIFI_AP_STATE)
	{
		psk_indicate_evt_hapd(priv, id, mac, msg, len);
		return;
	}	
