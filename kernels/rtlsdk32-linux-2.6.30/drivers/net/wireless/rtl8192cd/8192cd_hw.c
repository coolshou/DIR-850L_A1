...
#define LMT_NUM 256
static int get_chnl_lmt_dot_8812(struct rtl8192cd_priv *priv, unsigned char *line_head, unsigned int *channel, unsigned int *limit, unsigned int *table_idx)
{
...
{	
	...
	if(*u4bRegOffset == rTxAGC_A_CCK11_CCK1_JAguar)
		priv->pshare->tgpwr_CCK_8812[RF_PATH_A] = tmp_byte[3];

	if(*u4bRegOffset == rTxAGC_B_CCK11_CCK1_JAguar)
		priv->pshare->tgpwr_CCK_8812[RF_PATH_B] = tmp_byte[3];	

	//OFDM-54M
	if(*u4bRegOffset == rTxAGC_A_Ofdm54_Ofdm24_JAguar)
		priv->pshare->tgpwr_OFDM_8812[RF_PATH_A] = tmp_byte[3];

	if(*u4bRegOffset == rTxAGC_B_Ofdm54_Ofdm24_JAguar)
		priv->pshare->tgpwr_OFDM_8812[RF_PATH_B] = tmp_byte[3];	

	//HT-MCS7
	if(*u4bRegOffset == rTxAGC_A_MCS7_MCS4_JAguar)
		priv->pshare->tgpwr_HT1S_8812[RF_PATH_A] = tmp_byte[3];

	if(*u4bRegOffset == rTxAGC_B_MCS7_MCS4_JAguar)
		priv->pshare->tgpwr_HT1S_8812[RF_PATH_B] = tmp_byte[3];	

	//HT-MCS15
	if(*u4bRegOffset == rTxAGC_A_MCS15_MCS12_JAguar)
		priv->pshare->tgpwr_HT2S_8812[RF_PATH_A] = tmp_byte[3];

	if(*u4bRegOffset == rTxAGC_B_MCS15_MCS12_JAguar)
		priv->pshare->tgpwr_HT2S_8812[RF_PATH_B] = tmp_byte[3];	

	//VHT-NSS1-MCS7
	if(*u4bRegOffset == rTxAGC_A_Nss1Index7_Nss1Index4_JAguar)
		priv->pshare->tgpwr_VHT1S_8812[RF_PATH_A] = tmp_byte[3];

	if(*u4bRegOffset == rTxAGC_B_Nss1Index7_Nss1Index4_JAguar)
		priv->pshare->tgpwr_VHT1S_8812[RF_PATH_B] = tmp_byte[3];	

	//VHT-NSS2-MCS7
	if(*u4bRegOffset == rTxAGC_A_Nss2Index9_Nss2Index6_JAguar)
		priv->pshare->tgpwr_VHT2S_8812[RF_PATH_A] = tmp_byte[1];

	if(*u4bRegOffset == rTxAGC_B_Nss2Index9_Nss2Index6_JAguar)
		priv->pshare->tgpwr_VHT2S_8812[RF_PATH_B] = tmp_byte[1];	
...
unsigned int generate_u4bRegValue_8812_new(struct rtl8192cd_priv *priv, unsigned int *u4bRegOffset, unsigned char *tmp_byte)
{
	...
	{
		tmp_reg[0]= priv->pshare->tgpwr_VHT1S_8812[RF_PATH_A] - tmp_byte[0];
		tmp_reg[1]= priv->pshare->tgpwr_VHT1S_8812[RF_PATH_A] - tmp_byte[1];
		tmp_reg[2]= tmp_byte[2]- priv->pshare->tgpwr_VHT2S_8812[RF_PATH_A];
		tmp_reg[3]= tmp_byte[3]- priv->pshare->tgpwr_VHT2S_8812[RF_PATH_A];
	...
	{
		tmp_reg[0]= priv->pshare->tgpwr_VHT1S_8812[RF_PATH_B] - tmp_byte[0];
		tmp_reg[1]= priv->pshare->tgpwr_VHT1S_8812[RF_PATH_B] - tmp_byte[1];
		tmp_reg[2]= tmp_byte[2]- priv->pshare->tgpwr_VHT2S_8812[RF_PATH_B];
		tmp_reg[3]= tmp_byte[3]- priv->pshare->tgpwr_VHT2S_8812[RF_PATH_B];
	...
	switch (*u4bRegOffset) {
		...
		case rTxAGC_B_Nss2Index5_Nss2Index2_JAguar: 
			target = priv->pshare->tgpwr_VHT2S_8812[RF_PATH_B];
			break;
...
static int get_target_val_8812_new(struct rtl8192cd_priv *priv, unsigned char *line_head, unsigned int *u4bRegOffset, unsigned int *u4bMask, unsigned int *u4bRegValue)
{
	int base, idx;
	int num=0;
	unsigned char *ch;
...
static int get_offset_mask_val_8812_new(struct rtl8192cd_priv *priv, unsigned char *line_head, unsigned int *u4bRegOffset, unsigned int *u4bMask, unsigned int *u4bRegValue)
{
	int base, idx;
	int num=0;
	unsigned char *ch;
...
#ifdef TXPWR_LMT_8812
void find_pwr_limit_8812(struct rtl8192cd_priv *priv, int offset)
{
...
int PHY_ConfigTXLmtWithParaFile_8812(struct rtl8192cd_priv *priv)
{
	...
	struct TxPwrLmtTable_8812 *reg_table;
	...
	while (1) {
		...
		found = get_chnl_lmt_dot_8812(priv, line_head, &channel, &limit, &table_idx);
		...
		if (found > 0) {
			...
			len++;
			if ((len * sizeof(struct TxPwrLmtTable_8812)) > MAC_REG_SIZE)
				break;
		}
	}
...
int PHY_ConfigTXLmtWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	if((GET_CHIP_VER(priv)==VERSION_8192C) || (GET_CHIP_VER(priv)==VERSION_8188C))
	{
		...
		if(priv->pmib->dot11StationConfigEntry.txpwr_lmt_index <= 0) {
			...
			if(priv->pmib->dot11StationConfigEntry.dot11RegDomain==DOMAIN_FCC)
				...
				txpwr_lmt_index = TXPWR_LMT_MAX + 1;
		 }
		 else{
			txpwr_lmt_index = priv->pmib->dot11StationConfigEntry.txpwr_lmt_index;
		}

		if(txpwr_lmt_index <= TXPWR_LMT_MAX &&
				data_TXPWR_LMT_array[txpwr_lmt_index]) 
		{

			next_head = data_TXPWR_LMT_array[txpwr_lmt_index];
			read_bytes = data_TXPWR_LMT_array_end[txpwr_lmt_index] - data_TXPWR_LMT_array[txpwr_lmt_index];
		} 
		else 
		{		   		
	...
	if(GET_CHIP_VER(priv)==VERSION_8192D)
	{
		   ...
		   data_TXPWR_LMT_array[txpwr_lmt_index]) 
		{
			...
			next_head = data_TXPWR_LMT_array[txpwr_lmt_index];
			read_bytes = data_TXPWR_LMT_array_end[txpwr_lmt_index] -
						 data_TXPWR_LMT_array[txpwr_lmt_index];
		} else 
		{		   		
...
//_TXPWR_REDEFINE in MP Tool, 3 Groups: 36-99 100-148 149-165 
...

	{
		...
		if((GET_CHIP_VER(priv) == VERSION_8812E) && 
				(reg_file == PHYREG_PG) &&
				(!priv->pshare->rf_ft_var.disable_txpwrlmt))
		{
			...
			while(1) {
				...
				if (line_head[0] == '/')
					continue;				
					get_target_val_8812_new(priv, line_head, &u4bRegOffset, &u4bRegMask ,&u4bRegValue);
			}
		}
		...
		while (1) {
			...
			if (file_format == TWO_COLUMN) {
				...
				if((GET_CHIP_VER(priv) == VERSION_8812E) && 
					(reg_file == PHYREG_PG) &&
				 	(!priv->pshare->rf_ft_var.disable_txpwrlmt))
				num = get_offset_mask_val_8812_new(priv, line_head, &u4bRegOffset, &u4bRegMask ,&u4bRegValue);
				else
#endif
				num = get_offset_mask_val(line_head, &u4bRegOffset, &u4bRegMask ,&u4bRegValue);
				if (num > 0) {
...
#ifdef TXPWR_LMT_8812
		if (GET_CHIP_VER(priv) == VERSION_8812E)
		find_pwr_limit_8812(priv, offset);
		else
#endif
		find_pwr_limit(priv);
	}
...
*/
	...
	{
		RTL_W16(ATIMWND, 0x10); //Realtek for Intel 6205 IOT, Builder, 2013/01/29.
...
//Realtek for Intel 6205 IOT, Builder, 2013/01/29.
//+++
#if defined(CONFIG_RTL_8812_SUPPORT)
		if(GET_CHIP_VER(priv)==VERSION_8812E)
			RTL_W8(BCN_MAX_ERR, 0x00);
		else
#endif
//+++
		RTL_W8(BCN_MAX_ERR, 0xff);
...
#ifdef TXPWR_LMT_8812 
	if(GET_CHIP_VER(priv) == VERSION_8812E)	
		PHY_ConfigTXLmtWithParaFile_8812(priv);
	else
#endif
