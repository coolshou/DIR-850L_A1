...
//	range->max_r_time;	/* Maximal retry lifetime */
	...
	//range->num_channels = priv->available_chnl_num; /*Alpha, ---*/
	range->num_channels = priv->all_available_chnl_num; /*Alpha, +++*/
	//range->num_frequency = priv->available_chnl_num;  /*Alpha, ---*/
	range->num_frequency = priv->all_available_chnl_num;  /*Alpha, +++*/
	
	//for (i = 0; i < priv->available_chnl_num && i < IW_MAX_FREQUENCIES; i++) { /*Alpha, ---*/
	for (i = 0; i < priv->all_available_chnl_num && i < IW_MAX_FREQUENCIES; i++) { /*Alpha, +++*/
		u32 m = 0;
		range->freq[i].i = i + 1;
		//range->freq[val].m = CH_HZ_ID_MAP[i].freqKHz * 100000;
		//MAP_CHANNEL_ID_TO_KHZ(priv->available_chnl[i], m); /*Alpha, ---*/
		MAP_CHANNEL_ID_TO_KHZ(priv->all_available_chnl[i], m); /*Alpha, +++*/
...
//polo end
...

	{
		...
		iwe.u.qual.qual = priv->site_survey.bss[i].sq;
		//Alpha end
