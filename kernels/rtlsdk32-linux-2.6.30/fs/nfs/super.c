	...
	case 6:
		...
		if (data->context[0]){
			...
			if (rc)
				return rc;
#else
			//+++ fix by siyou 2010/5/17 06:50¤U¤È
			//no need to return fail just because context[] not be correct init.
			//if we do not define SELINUX, we should just ignore this checking instead to return false.
			//return -EINVAL;
#endif
		}
