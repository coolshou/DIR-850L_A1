...
Authenticate.prototype =
{
	...
	Login_Hash: function(callback, user, passwd, captcha)
	{
		if (this.AuthorizedGroup >= 0)
		{
			if (callback) callback(null);
		...
		AJAX.sendRequest("authentication.cgi?captcha="+captcha+"&dummy="+dummy, "");
	},
	Login_Send_Digest: function(callback, user, passwd, json)
	{
		var JsonData = eval('(' + json + ')');
		if(JsonData.status!="ok") // JsonData.status is not ok may due to captcha error.
		{
		...
		var payload = "id="+user+"&password="+DIGEST;
