...
//startcmd("insmod /lib/modules/rt2860v2_ap.ko");
...
while ($i>0)
{
	...
	else
	{
		...
		else
		{
			...
			if(isguestzone($uid)=="1")
			{
				...
				startcmd("echo 0 > /proc/sys/net/ipv6/conf/".$brdev."/disable_ipv6");
			}
	...
	/* enable Beamforming */
	startcmd("iwpriv ".$dev." set ITxBfEn=1");
	startcmd("iwpriv ".$dev." set ETxBfEnCond=1");
	/* enable Smart antena */
	/* set to auto mode */
	startcmd("iwpriv ".$dev." set sa_mode=3");
	startcmd("iwpriv ".$dev." set sa=1");
