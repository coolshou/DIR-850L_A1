...
{
	$p 		= XNODE_getpathbytarget("", "inf", "phyinf", $uid, 1);
	$inf 	= query($p."/uid");
	$inf_prefix = cut($inf, 0,"-");
	if($inf_prefix == "WAN") return "WAN";
	else 					 return "LAN";	 	
}
...
function PHYINF_setup($uid, $type, $inf)
{
	...
	$ifname = getnamefromuid($uid);
	setattr($path."/stats/tx/bytes",	"get","rtlioc getbytes | scut -p ".$ifname." -f 1 ");
	setattr($path."/stats/tx/packets",	"get","rtlioc getcounter  | scut -p ".$ifname." -f 1 ");	
	setattr($path."/stats/rx/bytes",	"get","rtlioc getbytes | scut -p ".$ifname." -f 2 ");
	setattr($path."/stats/rx/packets",	"get","rtlioc getcounter | scut -p ".$ifname." -f 2 ");	
