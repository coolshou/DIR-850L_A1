<div class="orangebox">
	...
	<ul>
		...
		<li><a href="#Storage"><?echo i18n("Storage");?></a></li>
		<?if ($FEATURE_PARENTALCTRL == "1") echo '<li><a href="#ParentCtrl">'.i18n("Parental Control").'</a></li>\n';?>	
		<?if ($FEATURE_NOIPV6 == "0") echo '<li><a href="#IPv6">'.i18n("IPv6").'</a></li>\n';?>
		<?if ($FEATURE_NOSMS =="0") echo '<li><a href="#SMS">'.i18n("Message Service").'</a></li>\n';?>
		<li><a href="#Mydlink"><?echo i18n("Mydlink Settings");?></a></li> 
	</ul>
</div>
<div class="blackbox">
					...
					<dl>
						...
						<dd><?
							...
							<p><?
								...
								echo i18n("The MAC (Media Access Control) Address field is required by some Internet Service Providers (ISP). The default MAC address is set to the MAC address of the WAN interface on the router. You can use the 'Clone Your PC's MAC Address' button to automatically copy the MAC address of the Ethernet Card installed in the computer used to configure the device. It is only necessary to fill in the field if required by your ISP.");
							?></p>
							<p><?
						...
						<dd><?
							...
							<p><?
								echo '<strong>'.i18n('MAC Address').'</strong> ('.i18n('optional').')<br/>';
								echo i18n("The MAC (Media Access Control) Address field is required by some Internet Service Providers (ISP). The default MAC address is set to be the MAC address of the WAN interface on the router. You can use the 'Clone Your PC's MAC Address' button to automatically copy the MAC address of the Ethernet Card installed in the computer that is being used to configure the device. It is only necessary to fill in this field if required by your ISP.");
							?></p>
							<p><?
								echo '<strong>'.i18n('Primary DNS Address').'</strong><br/>';
...
<div class="blackbox">
	...
	<dl>
		...
		<dt><?echo i18n('WDS');?></dt>
		<dd><?
			echo i18n("WDS is a system that enables the wireless network coverage of a router to be expanded using another router acting as a bridge. The extended network can use the router's original SSID or a new one created by you.");
		?></dd>
		-->
		...
		<dd><?
			...
			<dl>
				<dt><?echo i18n('WEP');?></dt>
				<dd><?
					echo i18n('A method of encrypting data for wireless communication intended to provide the same level of privacy as a wired network. WEP is not as secure as WPA encryption. To gain access to a WEP network, you must know the key. The key is a string of characters that you create. When using WEP, you must determine the level of encryption. The type of encryption determines the key length. 128-bit encryption requires a longer key than 64-bit encryption. Keys are defined by entering in a string in HEX (hexadecimal - using characters 0-9, A-F) or ASCII (American Standard Code for Information Interchange - alphanumeric characters) format. ASCII format is provided so you can enter a string that is easier to remember. The ASCII string is converted to HEX for use over the network. One keys can be defined so that you can change keys easily. A default key is selected for use on the network.');
					?>
					<div class="help_example">
					<dl>
						<dt><strong><?echo i18n("Example");?>: </strong></dt>
						<dd><?
						echo i18n('64-bit hexadecimal keys are exactly 10 characters in length. (12345678FA is a valid string of 10 characters for 64-bit encryption.)');
						?></dd>
						<dd><?
						echo i18n('128-bit hexadecimal keys are exactly 26 characters in length. (456FBCDF123400122225271730 is a valid string of 26 characters for 128-bit encryption.)');
						?></dd>
						<dd><?
						echo i18n('64-bit ASCII keys are up to 5 characters in length (DMODE is a valid string of 5 characters for 64-bit encryption.)');
						?></dd>
						<dd><?
						echo i18n('128-bit ASCII keys are up to 13 characters in length (2002HALOSWIN1 is a valid string of 13 characters for 128-bit encryption.)');
						?></dd>
					</div>
				</dd>
				...
				<dt><?echo i18n('WPA Mode: ');?></dt>
					<dd><?
						echo i18n('WPA is the older standard; select this option if the clients that will be used with the router 
									...
									With the "Auto" option, the router tries WPA2 first, but falls back to WPA if the client only supports WPA. 
									With the "WPA2 Only" option, the router associates only with clients that also support WPA2 security.
									With the "WPA Only" option, the router associates only with clients that also support WPA security.');
					?></dd>
	...
	<dl>
		...
		<dd><?
			...
			<dl>
				...
				?></dd>
				<dt><?echo i18n('DHCP Lease Time');?></dt>
				<dd><?
		...
		<dt><?echo i18n('Add DHCP Reservation');?></dt>
		<dd><? 
...
<div class="blackbox">
	...
	<dl>
		...
		<dd><?
			echo i18n('If device setting DDNS and plug one USB device, the link should show http://DDNS/:8181, if it does not setting DDNS then show http://WAN IP Address/:8181 in "HTTP Storage Link", user can use this link to connect to the drive remotely after logging in with a user account.');
		?></dd>		
	<dl>
</div>
<div class="blackbox" <?if ($FEATURE_PARENTALCTRL == "0") echo ' style="display:none;"';?>>
	<h2><a name="ParentCtrl"><?echo i18n("Parental Control");?></a></h2>
...
<div class="blackbox">
	...
	<dl>
		...
		<dd>
			<?
			echo i18n('There are several connection types to choose from: Link-local Only, Auto Detection, Static IPv6, Autoconfiguration(SLAAC/DHCPv6), PPPoE, IPv6 in IPv4 Tunnel, 6to4,6rd and Local connectivity Only. If you are unsure of your connection method, please contact your IPv6 Internet Service Provider. Note: If using the PPPoE option, you will need to ensure that any PPPoE client software on your computers has been removed or disabled.');
			?>
			<dl>
