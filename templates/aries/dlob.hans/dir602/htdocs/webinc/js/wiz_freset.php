...
Page.prototype =
{
		...
		if(confsize==0 && this.logindefault == 0)
		{
			...
			AUTH.Login(
				function login_callback()
				{
					...
					{
						alert("<?echo i18n("password is not default(Admin/blank),should not run here.");?>");
						BODY.OnLoad();
					}
				},
				"Admin","");
	...
	OnClickNext: function()
	{
		...
		else if (stage == "stage_wlan_set")
		{
			...
			for(var i=0;i < OBJ("wiz_ssid").value.length;i++)
			{
				var char_input=OBJ("wiz_ssid").value.charCodeAt(i) ;
				if (char_input < 32 || char_input >126) 
				{ 
					BODY.ShowAlert("<?echo i18n("Invalid Wi-Fi Network Name (SSID).");?>");
					return false;
				}
	...
	OnClickCancel: function()
	{
		...
		{
			self.location.href = "st_device.php";
