...
{	
	...
	OnSubmitCallback: function (code, result)
	{
		...
		{
		case "OK":
			var wds_mod = OBJ("en_wifi_WDS").getAttribute("modified");	
			if (COMM_EqBOOL(OBJ("en_wifi_WDS").getAttribute("modified"), true))
			{
				if (OBJ("en_wifi_WDS").checked)
				{
						/* change to bridge mode and use DHCP. */
						var msgArray =
						[
							'<?echo i18n("The device is changing to WDS mode.");?>',
							...
							'<a href="http://<?=$hostnamemac?>" style="color:#0000ff;">http://<?=$hostnamemac?></a>'
						];
						url = "http://"+hostnamemac+"/index.php";
				}
				else
				{
					...
					[
						...
						'<?echo i18n("You can access the device by clicking the link below.");?>',
						'<a href="http://<?=$hostname?>" style="color:#0000ff;">http://<?=$hostname?></a>'
					];
					url = "http://"+hostname+"/index.php";
				}
				BODY.ShowCountdown('<?echo i18n("Device Mode");?>', msgArray, this.bootuptime, url);
			}
			else
			{
				BODY.OnReload();
	...
	InitValue: function(xml)
	{
		...
		if(!this.Initial("BAND24G-1.3","WIFI.PHYINF")) return false;
	...
	{		
		...
		if(!this.ValidityCheck("BAND24G-1.3")) return null;	
		if(!this.SaveXML("BAND24G-1.3")) return null;
		if(this.dual_band)
		{
	...
	Initial: function(wlan_uid,wifi_module)
	{
		...
		if(opmode == "APCLI")	str_Aband = "_WDS";
		...
		if(opmode =="APCLI") 
		{
			if (XG(this.wifip+"/ssidremain")=="1")
			{
				OBJ("ssid_Remain").checked = true;
			}
			else
			{
				OBJ("ssid_Create").checked = true;
			}
			this.OnClickExtssid();
			this.OnClickEnWDS();
		}
	...
	WPSCHK: function(wlan_uid)
	{
		...
		OBJ("en_wifi_WDS").checked && OBJ("ssid_Remain").checked)
		{
			XS(wifip+"/wps/configured", "1");
		}
		...
		if(wps_enable)
		{
			...
			{
				if(confirm('<?echo I18N("j", "To use hidden SSID (invinsible), WPS must be disabled. Proceed ? ");?>'))
					//XS(wifip+"/wps/enable", "0");
					this.SetWps("disable");
				else 
					return false;
	...
	SaveXML: function(wlan_uid)
	{
		...
		{
			XS(phyinf+"/active", "1");
			if(opmode == "APCLI") XS(layout, "bridge");

		}	
		else
		{
			XS(phyinf+"/active", "0");
			if(opmode == "APCLI") XS(layout, "router");
			return true;
		}
		...
		if(opmode == "APCLI")
		{
			if(OBJ("ssid_Remain").checked)
			{
				XS(wifip+"/ssidremain", "1");
				XS(wifip+"/ssid",       OBJ("ssid_WDS").value);
			}
			else
			{
				XS(wifip+"/ssidremain", "0");
			}
	...
	OnClickExtssid: function()
	{
		if (OBJ("en_wifi_WDS").checked) {
			if (OBJ("ssid_Remain").checked)
			{
				OBJ("ssid").value = OBJ("ssid_WDS").value;
	...
	OnClickEnWDS: function()
	{
		if (AUTH.AuthorizedGroup >= 100) return;
		var  div_24G_mem = new Array("div_24G_wlband", "div_24G_enwlan", "div_24G_wlmode", "div_24G_autoch", "div_24G_channel", "div_24G_txrate", "div_24G_visibility" );
		var div_security_options = new Array("wep_key_len_WDS", "auth_type_WDS", "wep_64_1_WDS", "wep_128_1_WDS", "wpa_mode_WDS", "cipher_type_WDS", "wpa_psk_key_WDS");
		var memdisp;
		var sectdisabled;
		if (OBJ("en_wifi_WDS").checked)
		{
			OBJ("div_24G_title").innerHTML ='<?echo i18n("Extended Wireless Network Settings");?>';
			memdisp="none";
			OBJ("div_24G_extssid").style.display="block";
			OBJ("div_security_title").innerHTML = '<?echo i18n("Extended Wireless Security Mode");?>';
			if(OBJ("security_type").options.item(3)) {
				OBJ("security_type").options.remove(3);
			}
			OBJ("wlan24G_ssidname").innerHTML ='';
			OBJ("wlan24G_ssiddelimiter").innerHTML ='';

			OBJ("ssid_WDS").disabled = false;
			OBJ("security_type_WDS").disabled = false;
			OBJ("site_survey").disabled = false;
			if(this.feature_nosch==0)
			{
				OBJ("sch_WDS").disabled = false;
				OBJ("go2sch_WDS").disabled = false;
			}
			OBJ("auto_ch").checked = false;
			sectdisabled = false;
		}
		else
		{
			OBJ("div_24G_title").innerHTML ='<?echo i18n("Wireless Network Settings");?>';
			memdisp="block";
			OBJ("div_24G_extssid").style.display="none";
			OBJ("div_security_title").innerHTML = '<?echo i18n("Wireless Security Mode");?>';
			if(!OBJ("security_type").options.item(3)) {
				OBJ("security_type").options.add(new Option("<?echo i18n("WPA-Enterprise");?>","wpa_enterprise"));
			}
			OBJ("wlan24G_ssidname").innerHTML ='<?echo i18n("Wireless Network Name");?>';
			OBJ("wlan24G_ssiddelimiter").innerHTML =':';

			OBJ("ssid_WDS").disabled = true;
			OBJ("security_type_WDS").disabled = true;
			OBJ("site_survey").disabled = true;
			if(this.feature_nosch==0)
			{
				OBJ("sch_WDS").disabled = true;
				OBJ("go2sch_WDS").disabled = true;
			}
			sectdisabled = true;
		}
			for(var i=0; i < div_24G_mem.length; i++)
			{
				OBJ(div_24G_mem[i]).style.display= memdisp;
			}

			for(var i=0; i < div_security_options.length; i++)
			{
				OBJ(div_security_options[i]).disabled = sectdisabled;
			}
	},
...
//			this.OnChangeWLMode(str_Aband);	
	...
	OnChangeWEPAuth: function(str_Aband)
	{
		...
		{
			BODY.ShowAlert("<?echo I18N("h", "Can't choose shared key when wps is enable !!");?>");
	...
	OnClickRadiusAdvanced: function(str_Aband)
    {
        if (this.radius_adv_flag) {
            ...
            OBJ("radius_adv"+str_Aband).value = "<?echo i18n("Advanced");?>>>";
