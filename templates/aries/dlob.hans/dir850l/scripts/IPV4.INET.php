...
function dev_detach($hasevent)
{
	...
	//if the address is same as alias we should up the alias
	...
	if($cnt!="" && $cnt!="0") 
	{
		...
		{
			if($InDeX > $cnt) break;
			if($VaLuE!="" && $ipaddr==$VaLuE) 
			{
				echo "ifconfig ".$devnam.":".$InDeX." ".$VaLuE." up\n"; 	
			}	
		}
	}
	del($sts."/inet");
...
function dev_attach($hasevent)
{
	...
	//if the address is same as alias we should down the alias
	$cnt = query($sts."/ipalias/cnt")+0; 
	if($cnt!="" && $cnt!="0") 
	{
		foreach($sts."/ipalias/ipv4/ipaddr") 	
		{
			if($InDeX > $cnt) break;
			if($VaLuE!="" && $_GLOBALS["IPADDR"]==$VaLuE) 
			{
				echo "ifconfig ".$_GLOBALS["DEVNAM"].":".$InDeX." down\n"; 	
			}	
		}
	}
