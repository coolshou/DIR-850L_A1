...
function verify_portforward($path, $max)
{
	...
	foreach ($path."/entry")
	{
		...
		if ($start=="")
			return result("FAILED", $entry."/external/start", i18n("Please enter public port."));
		if (isvalid_port($start)!="YES")
			return result("FAILED", $entry."/external/start",
				i18n("Invalid public port value."));
		...
		if ($end!="")
		{
			if (isvalid_port($end)!="YES")
				return result("FAILED", $entry."/external/end",
					i18n("Invalid public port range value."));
			if ($start > $end)
				return result("FAILED", $entry."/external/start",
					i18n("Invalid public port value.")." ".
		...
		if ($hostid=="" || isdigit($hostid)==0)
			return result("FAILED", $entry."/internal/hostid", i18n("Invalid internal host."));
...
function verify_porttrigger($path, $max)
{
	...
	foreach ($path."/entry")
	{
		...
		if ($val!="TCP" && $val!="UDP" && $val!="TCP+UDP")
			return result("FAILED", $entry."/trigger/protocol", i18n("Invalid application protocol."));

		/* Check the trigger starting port. */
		$start = query("trigger/start");
		if ($start=="")
			return result("FAILED", $entry."/trigger/start", i18n("Please enter application port."));
		if (isvalid_port($start)!="YES")
			return result("FAILED", $entry."/trigger/start",
				i18n("Invalid application port value."));
		...
		if ($end!="")
		{
			if (isvalid_port($end)!="YES")
				return result("FAILED", $entry."/trigger/end",
					i18n("Invalid application port range value."));
			if ($start > $end)
				return result("FAILED", $entry."/trigger/start",
					i18n("Invalid application port value.")." ".
					i18n("The ending port should be greater than the beginning port."));
...
function verify_dmz($path)
{
	...
	if ($enable=="1")
	{
		...
		if ($hostid=="" || isdigit($hostid)==0)
			return result("FAILED", $entry."/hostid", i18n("Invalid internal DMZ host."));
