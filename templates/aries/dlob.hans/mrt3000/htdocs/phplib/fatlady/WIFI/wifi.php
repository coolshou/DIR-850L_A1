	...
	foreach($p."/eap")
	{
		...
		if (INET_validv4addr(query($p."/eap:".$InDeX."/radius"))!=1)
			return result("FAILED", $p."/eap:".$InDeX."/radius",
				i18n("Invalid RADIUS IP address."));
...
function check_wapias($p)
{
	...
	if (INET_validv4addr(query($p."/wapi/as"))!=1)
		return result("FAILED", $p."/wapi/as",
				i18n("Invalid authentication server IP address."));
...
function check_media($band, $p)
{
		...
		$wlmode == "bgn")
	{
		...
		return result("FAILED", $p."/wlmode",
				i18n("Invalid wireless mode.")." (".$wlmode.")");
	/* beacon */
	$v = query($p."/beacon");
	TRACE_debug("FATLADY: beacon=".$v);
	if ($v<20 || $v>1000)
		return result("FAILED", $p."/beacon",
				i18n("Invalid beacon interval value")." ".
	...
	if ($v<256 || $v>2346)
		return result("FAILED", $p."/rtsthresh",
					i18n("Invalid RTS threshold value.")." ".
	...
	if ($v != 0)
	{
		if ($band=="BAND5G") 
		{
			...
			if (check_dfs($v)!=1) 
			{
				return result("FAILED", $p."/channel", i18n("Selected channel is blocked. Please select another channel !"));	
			}
		}
		else 
			$band="g";
...
function fatlady_wifi($prefix, $phyinf)
{
	...
	if ($len>32) return result("FAILED", $p."/ssid", i18n("The maximum length of SSID should be 32."));
	/* ssidhidden */
