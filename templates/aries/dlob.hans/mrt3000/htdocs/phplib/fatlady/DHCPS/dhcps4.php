...
function check_dhcp_config($path, $inetp)
{
	...
	if ($start_ip == 0)
	{
		set_result("FAILED",$path."/start",i18n("The input range of host id is out of boundary."));
		return;
	}
	if ($end_ip >= ipv4maxhost($mask))
	{
		set_result("FAILED",$path."/end",i18n("The input range of host id is out of boundary."));
		return;
	}
	...
	{
		set_result("FAILED",$path."/leasetime",i18n("The  lease time cannot be less than 1 minute."));
		return;
	}
	...
	{
		set_result("FAILED",$path."/router",i18n("Invalid router address"));
		return;
	}
	...
	while ($i < $cnt)
	{
		...
		{
			set_result("FAILED",$path."/dns/entry:".$i,i18n("Invalid DNS Server addres."));
			return;
		}
	}
	...
	while ($i < $cnt)
	{
		...
		{
			set_result("FAILED",$path."/wins/entry:".$i,i18n("Invalid WINS server address"));
			return;
		}
	}
