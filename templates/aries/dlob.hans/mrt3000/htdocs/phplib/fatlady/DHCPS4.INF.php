...
function verify_dhcps4($path)
{
	...
	if ($start > $end)
		return result("FAILED", $path."/start",
				i18n("The start of the DHCP lease pool should be equal to or less than the end."));

	if ($start==0)
		return result("FAILED", $path."/start", i18n("The DHCP lease pool cannot be zero."));
	...
	if ($router!="" && ipv4networkid($router,32)=="")
		return result("FAILED", $path."/router", i18n("Invalid router address"));
	...
	foreach("dns/entry")
	{
		...
		if (ipv4networkid($VaLuE,32)=="")
			return result("FAILED", $path."/dns/entry:".$InDeX, i18n("Invalid DNS address."));
	}
	/* check WINS */
	...
	foreach("wins/entry")
	{
		if ($InDeX>$cnt) break;
		if (ipv4networkid($VaLuE, 32)=="")
			return result("FAILED", $path."/wins/entry:".$InDeX, i18n("Invalid WINS address."));
	}
