...
Page.prototype =
{
	...
	{		
		...
		if (rt != "")
		{
			...
			case "SUCCESS":				
				...
				var msg_str = "<?echo I18N("j","The time has been successfully synchronized.");?>";
				msg_str += "<br>(<?echo I18N("j","NTP Server Used: ");?>" + st_server + "<?echo I18N("j",", Time: ");?>"+st_uptime+")";
				msg_str += "<br><?echo I18N("j","Next time synchronization: ");?>"+st_nexttime;
				OBJ("st_time").innerHTML = xml.Get(rt+"/runtime/device/uptime");
				...
				break;
			case "RUNNING":
				OBJ("sync_msg").innerHTML = "<?echo I18N("j","Synchronizing ...");?>";
				this.IsSynchronized = "";
	...
	OnClickNTPSync: function()
	{
		...
		if(OBJ("ntp_server").value==="")
		{
			...
			BODY.ShowConfigError("ntp_server", "<?echo I18N("j","Invalid NTP server !");?>");
			return false;
		}
		OBJ("sync_msg").innerHTML = "<?echo I18N("j","Synchronizing ...");?>";
		clearTimeout(PAGE.tid1);
	...
	{		
		...
		{												
			...
			case "SUCCESS":				
				OBJ("st_time").innerHTML = xml.Get(rt+"/runtime/device/uptime");				
				OBJ("sync_pc_msg").innerHTML = "<?echo I18N("j","");?>";
				clearTimeout(PAGE.tid1);
				this.tid2 = setTimeout('PAGE.GetSyncTimeStatus()', 1000);
				break;
			case "RUNNING":
				OBJ("sync_pc_msg").innerHTML = "<?echo I18N("j","Synchronizing ...");?>";
