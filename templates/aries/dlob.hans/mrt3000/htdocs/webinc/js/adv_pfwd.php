...
Page.prototype =
{
	...
	* Usually the BODY will handle callback of submit. */
	...
	InitValue: function(xml)
	{
		...
		if (p === "") BODY.ShowMessage("<?echo I18N("j","Error");?>","ERROR!");
	...
	PreSubmit: function()
	{
		...
		for (var i=1; i<=<?=$PFWD_MAX_COUNT?>; i+=1)
		{
			...
			{
				BODY.ShowConfigError("port_tcp_"+i,"<?echo I18N("j", "Invalid TCP port range.");?>");
				OBJ("port_tcp_"+i).focus();
				return null;
			}
			if (OBJ("port_udp_"+i).value!="" && !check_valid_port(OBJ("port_udp_"+i).value))
			{
				BODY.ShowConfigError("port_udp_"+i,"<?echo I18N("j", "Invalid UDP port range.");?>");
			...
			{
				BODY.ShowConfigError("port_tcp_"+i,"<?echo I18N("j", "Invalid Port !");?>");
				OBJ("port_tcp_"+i).focus();
				return null;
			}
		...
		for (var i=1; i<cur_count; i+=1)
		{
			for (var j=i+1; j<=cur_count; j+=1)
			{
				if(OBJ("ip_"+i).value!="" && OBJ("ip_"+j).value!="" && OBJ("port_tcp_"+i).value!="" && OBJ("port_tcp_"+j).value!="" &&
					OBJ("port_udp_"+i).value!="" && OBJ("port_udp_"+j).value!="" && OBJ("dsc_"+i).value != "" && OBJ("dsc_"+j).value !="") 
				{					
					if(OBJ("ip_"+i).value===OBJ("ip_"+j).value && OBJ("port_tcp_"+i).value===OBJ("port_tcp_"+j).value &&
						OBJ("port_udp_"+i).value===OBJ("port_udp_"+j).value) 
					{
						BODY.ShowConfigError("dsc_"+j,"<?echo I18N("j", "The rules cannot be the same.");?>");
						OBJ("dsc_"+j).focus();
						return null;
					}
					if(OBJ("dsc_"+i).value === OBJ("dsc_"+j).value && OBJ("dsc_"+j).value !=="") 
					{
						BODY.ShowConfigError("dsc_"+j,"<?echo I18N("j", "Different rules cannot have the same name.");?>");
						OBJ("dsc_"+j).focus();
						return null;
					}
				}	
