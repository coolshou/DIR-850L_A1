...
Page.prototype =
{
	...
	* Usually the BODY will handle callback of submit. */
	...
	InitValue: function(xml)
	{
		...
		//xml.dbgdump();
		if (!inet) { BODY.ShowMessage("<?echo I18N("j","Error");?>","InitValue ERROR!"); return false; }
																				
		if (this.activewan==="")
		{
			BODY.ShowMessage("<?echo I18N("j","Error");?>","<?echo I18N("j", "There is no interface can access the Internet!  Please check the cable, and the Internet settings!");?>");
			return false;
		}
		if (bwc === "")		{ BODY.ShowMessage("<?echo I18N("j","Error");?>","InitValue ERROR!"); return false; }		
		OBJ("en_qos").checked = ((XG(bwc+"/bwc/portbase_qos_enable")==="1" || XG(bwc+"/bwc/entry:1/enable")==="1")&& XG(bwc+"/runtime/device/layout")==="router");
		OBJ("en_qos").disabled =(XG(bwc+"/runtime/device/layout")==="bridge");
       	COMM_SetSelectValue(OBJ("QoS_Type"),XG(bwc+"/bwc/qos_type"));
        this.OnChangeQoSType("");
        COMM_SetSelectValue(OBJ("LanPort_1st") ,XG(bwc+"/bwc/port_priority/lanport1"));
        COMM_SetSelectValue(OBJ("LanPort_2nd") ,XG(bwc+"/bwc/port_priority/lanport2"));
        COMM_SetSelectValue(OBJ("LanPort_3rd") ,XG(bwc+"/bwc/port_priority/lanport3"));
        COMM_SetSelectValue(OBJ("LanPort_4th") ,XG(bwc+"/bwc/port_priority/lanport4"));
	...
	PreSubmit: function()
	{		
		XS(bwc+"/bwc/portbase_qos_enable", OBJ("en_qos").checked?"1":"0");	
		XS(bwc+"/bwc/entry:1/enable", OBJ("en_qos").checked?"1":"0");
		XS(bwc+"/bwc/qos_type", OBJ("QoS_Type").value);
		XS(bwc+"/bwc/port_priority/lanport1", OBJ("LanPort_1st").value);
		XS(bwc+"/bwc/port_priority/lanport2", OBJ("LanPort_2nd").value);
		XS(bwc+"/bwc/port_priority/lanport3", OBJ("LanPort_3rd").value);
		XS(bwc+"/bwc/port_priority/lanport4", OBJ("LanPort_4th").value);
		if(OBJ("QoS_Type").value === "lan_port")
		{
			//PXML.doc.dbgdump();
			//return null;
			return PXML.doc;	
		...
		{			
			BODY.ShowConfigError("upstream","<?echo I18N("j", "Invalid uplink speed.");?>");
			OBJ("upstream").focus();
			return null;
		}
		if (!TEMP_IsDigit(OBJ("downstream").value) || OBJ("downstream").value < 1)
		{			
			BODY.ShowConfigError("downstream","<?echo I18N("j", "The input downlink speed is invalid.");?>");
		...
		if(OBJ("Qtype_WFQ").checked === true)
		{
			for (var i=1; i<=4; i+=1)
			{
				...
				{
					BODY.ShowConfigError("priority_dsc"+i,"<?echo I18N("j", "The input queue weight is invalid.");?>");
			...
			{			
				BODY.ShowConfigError("priority_dsc1","<?echo I18N("j", "The total amount of queue weight should be");?>"+"100%.");
			...
			if(OBJ("dsc_"+i).value !== "")
			{
				for(var j=1; j <= <?=$QOS_MAX_COUNT?>; j++)
				{
					if(OBJ("dsc_"+j).value !== "")
					{
						...
						{
							BODY.ShowConfigError("dsc_"+i,"<?echo I18N("j","The 'Name' could not be the same.");?>");
							OBJ("dsc_"+i).focus();
							return null;			
						}
							...
							&& OBJ("dst_startip_"+i).value===OBJ("dst_startip_"+j).value && OBJ("dst_endip_"+i).value===OBJ("dst_endip_"+j).value)
						{
							...
							{	
								...
								{
									BODY.ShowConfigError("src_startip_"+i,"<?echo I18N("j","The rules could not be the same");?>");
		...
		for (var i=1; i<=<?=$QOS_MAX_COUNT?>; i+=1)
		{
			...
			{
				BODY.ShowConfigError("dsc_"+i,"<?echo I18N("j", "The 'Name' field can not be blank.");?>");
				OBJ("dsc_"+i).focus();
				return null;
			}
			if (OBJ("dsc_"+i).value !== "")
			{
				...
				{
					BODY.ShowConfigError("pro_"+i,"<?echo I18N("j", "Invalid protocol.");?>");
					OBJ("pro_"+i).focus();
					return null;
				}
	...
	bwc1link: null,
        ...
        OnChangeQoSType: function(QoSType)
        {
                if(QoSType === "")
                {
                        switch (OBJ("QoS_Type").value)
                        {
                                case "lan_port":
                                        ...
                                        OBJ("qos_setup").style.display = "none";
                                        OBJ("qos_setup_rules").style.display = "none";
                                        break;
                                case "protocol":
                                        OBJ("show_Port_Priority").style.display = "none";
                                        OBJ("qos_setup").style.display = "block";
                                        OBJ("qos_setup_rules").style.display = "block";
                                        break;
                        }
                }
                else if(QoSType === "lan_port") 
                {
                        OBJ("show_Port_Priority").style.display = "block";
                        OBJ("qos_setup").style.display = "none";
                        OBJ("qos_setup_rules").style.display = "none";
                }
                else if(QoSType === "protocol")
                {
                        OBJ("show_Port_Priority").style.display = "none";
                        OBJ("qos_setup").style.display = "block";
                        OBJ("qos_setup_rules").style.display = "block";
	...
	OnClickQOSEnable: function()
	{
		...
		{
                        OBJ("QoS_Type").disabled = false;
                        OBJ("LanPort_1st").disabled = OBJ("LanPort_2nd").disabled = OBJ("LanPort_3rd").disabled = OBJ("LanPort_4th").disabled = false;
		...
		{
                        OBJ("QoS_Type").disabled = true;
                        OBJ("LanPort_1st").disabled = OBJ("LanPort_2nd").disabled = OBJ("LanPort_3rd").disabled = OBJ("LanPort_4th").disabled = true;
	...
	OnClickQtype: function(Qtype)
	{
		if(Qtype === "SPQ")
		{	
			OBJ("priority").innerHTML = '<?echo I18N("h", "Queue Priority");?>';		
		...
		{
			OBJ("priority").innerHTML = '<?echo I18N("h", "Queue Weight");?>';
			OBJ("priority1").innerHTML = '<input id="priority_dsc1" type="text" size="3" maxlength="3" />% <?drawlabel("priority_dsc1");?>';
			OBJ("priority2").innerHTML = '<input id="priority_dsc2" type="text" size="3" maxlength="3" />% <?drawlabel("priority_dsc2");?>';
			OBJ("priority3").innerHTML = '<input id="priority_dsc3" type="text" size="3" maxlength="3" />% <?drawlabel("priority_dsc3");?>';
			OBJ("priority4").innerHTML = '<input id="priority_dsc4" type="text" size="3" maxlength="3" />% <?drawlabel("priority_dsc4");?>';					
	...
	IPRangeCheck: function(i)
	{
		...
		if(OBJ("src_startip_"+i).value !== "")
		{
			...
			{
				BODY.ShowConfigError("src_startip_"+i,"<?echo I18N("j", "The IP Address and LAN IP Address cannot be the same.");?>");
				OBJ("src_startip_"+i).focus();
				return false;
			}	
			...
			{	
				BODY.ShowConfigError("src_startip_"+i,"<?echo I18N("j", "IP address should be in LAN subnet.");?>");
				OBJ("src_startip_"+i).focus();
				return false;
			}
		}
		if(OBJ("src_endip_"+i).value !== "")
		{
			...
			{
				BODY.ShowConfigError("src_endip_"+i,"<?echo I18N("j", "The IP Address and LAN IP Address cannot be the same.");?>");
			...
			{	
				BODY.ShowConfigError("src_endip_"+i,"<?echo I18N("j", "IP address should be in LAN subnet.");?>");
				OBJ("src_endip_"+i).focus();
				return false;
			}
		}
		if(OBJ("src_startip_"+i).value !== "" && OBJ("src_endip_"+i).value !== "")
		{
			...
			{
				BODY.ShowConfigError("src_startip_"+i,"<?echo I18N("j", "The end IP address should be greater than the start address.");?>");
				OBJ("src_startip_"+i).focus();
				return false;
			}
			...
			{	
				BODY.ShowConfigError("src_startip_"+i,"<?echo I18N("j", "The start IP address and the end IP address should be in the same LAN subnet.");?>");
				OBJ("src_startip_"+i).focus();
				return false;
			}
		}
		...
		if(OBJ("dst_startip_"+i).value !== "" && OBJ("dst_endip_"+i).value !== "")
		{
			...
			{
				BODY.ShowConfigError("dst_startip_"+i,"<?echo I18N("j", "The end IP address should be greater than the start address.");?>");
				OBJ("dst_startip_"+i).focus();
				return false;
			}
		}
	...
	AppPortCheck: function(i)
	{
		if(TEMP_IsDigit(OBJ("app_port_"+i).value))
		{
			...
			{
				BODY.ShowConfigError("app_port_"+i,"<?echo I18N("j", "Invalid application port value.");?>");
...
{	
	...
	if (svc=="FRESET")		url = "http://192.168.0.1/home.php";
	else if (svc=="REBOOT")	url = "http://<?echo $_SERVER["HTTP_HOST"];?>/home.php";
	else					return false;
	...
	ajaxObj.onCallback = function (xml)
	{
		...
		if (xml.Get("/report/result")!="OK")
			BODY.ShowMessage("<?echo I18N("j","Error");?>","Internal ERROR!\nEVENT "+svc+": "+xml.Get("/report/message"));
		else
