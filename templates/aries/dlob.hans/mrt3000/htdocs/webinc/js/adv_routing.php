...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		if (p === "") { BODY.ShowMessage("Error","InitValues() ERROR !"); return false; }
		p += "/route/static";
	...
	PreSubmit: function()
	{
		...
		for(var i = 1;i <= <?=$ROUTING_MAX_COUNT?>;i++)
		{
			...
			if(en === "1" || name!="" || dip != "" || netmask != "" || gw != "")
			{
				if(name === "")
				{
					BODY.ShowMessage("Error","<?echo I18N("h", "The name cannot be blank");?>");
				...
				{
					BODY.ShowMessage("Error","<?echo I18N("h", "Invalid Destination IP address.");?>");
					BODY.ShowConfigError("dstip_"+i,"");
					OBJ("dstip_"+i).focus();
					return null;
				}
				for (var j=0; j<4; j++)
				...
				if(netmask === "" || mask === -1)
				{
					BODY.ShowMessage("Error","<?echo I18N("h", "Invalid Subnet Mask.");?>");
					BODY.ShowConfigError("netmask_"+i,"");
					OBJ("netmask_"+i).focus();
					return null;
				}
				if(gw === "")
				{
					BODY.ShowMessage("Error","<?echo I18N("h", "Invalid Gateway.");?>");
					BODY.ShowConfigError("gateway_"+i,"");
					OBJ("gateway_"+i).focus();
					return null;
				}				
				if(metric < 1 || 16 < metric || !TEMP_IsDigit(metric))
				{
					BODY.ShowMessage("Error","<?echo I18N("h", "The metric should be 1~16");?>");
					BODY.ShowConfigError("metric_"+i,"");
					OBJ("metric_"+i).focus();
					return null;
				}
				cur_count++;
		...
		for (var i=1; i<cur_count; i+=1)
		{
			for (var j=i+1; j<=cur_count; j+=1)
			{
				if(OBJ("name_"+i).value != "" && OBJ("name_"+j).value !="")
				{
					if(OBJ("name_"+i).value == OBJ("name_"+j).value) 
					{
						BODY.ShowMessage("Error","<?echo I18N("j", "Different rules cannot have the same name.");?>");
						BODY.ShowConfigError("name_"+i,"");
						OBJ("name_"+j).focus();
						OBJ("name_"+i).focus();
						return null;
