...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		if (p === "") BODY.ShowMessage("Error","<?echo I18N("j","ERROR!");?>");
	...
	PreSubmit: function()
	{
		...
		for (var i=1; i<=<?=$APP_MAX_COUNT?>; i+=1)
		{
			...
			for(j=1; j < i; j++)
			{
				...
				{
					BODY.ShowMessage("Error",'<?echo I18N("j","The Name ");?>"'+name+'\"<?echo I18N("j"," is already used !");?>');
					BODY.ShowConfigError("name_"+i, "");
					OBJ("name_"+i).focus();
					return null;
				}				
			}
			...
			{
				BODY.ShowMessage("Error","<?echo I18N("j","Please enter trigger port range !");?>");
				BODY.ShowConfigError("priport_"+i, "");
				OBJ("priport_"+i).focus();
				return null;
			}
			if (OBJ("priport_"+i).value!="" && !check_trigger_port(OBJ("priport_"+i).value))
			{
				BODY.ShowMessage("Error","<?echo I18N("j","Invalid Trigger port setting !");?>");
				BODY.ShowConfigError("priport_"+i, "");
				OBJ("priport_"+i).focus();
				return null;
			}
			...
			{
				BODY.ShowMessage("Error","<?echo I18N("j","Please enter firewall port range!");?>");
				BODY.ShowConfigError("pubport_"+i, "");
				OBJ("pubport_"+i).focus();
				return null;
			}
			if (OBJ("pubport_"+i).value!="" && !check_public_port(OBJ("pubport_"+i).value))
			{
				BODY.ShowMessage("Error","<?echo I18N("j","Invalid Firewall port setting !");?>");
				BODY.ShowConfigError("pubport_"+i, "");
				OBJ("pubport_"+i).focus();
				return null;
			}
			...
			for(j=1; j < i; j++)
			{
				...
				{
					BODY.ShowMessage("Error",'<?echo I18N("j","The Rule ");?>"'+name+'\"<?echo I18N("j","already exists!");?>');
					BODY.ShowConfigError("priport_"+i, "");
					OBJ("priport_"+i).focus();
					return null;
				}				
	...
	apps: [	{name: "<?echo I18N("j","Application Name");?>",
...
function OnClickAppArrow(idx)
{
	...
	if( i === "0" )
	{
		BODY.ShowMessage("Error","<?echo I18N("j","Please select an Application Name first !");?>");
		return false;
	}
...
function CheckPort(port)
{
	...
	{
	case 1:
		if (!TEMP_IsDigit(String(vals)))
			return false;
		break;
	case 2:
		if (!TEMP_IsDigit(String(vals[0]))||!TEMP_IsDigit(String(vals[1])))
			return false;
		break;
	default:
		return false;
