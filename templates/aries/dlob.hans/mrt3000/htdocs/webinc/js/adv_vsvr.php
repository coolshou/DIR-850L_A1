...
Page.prototype =
{
	...
	PreSubmit: function()
	{
		...
		for (var i=1; i<=<?=$VSVR_MAX_COUNT?>; i+=1)
		{
			...
			{
				BODY.ShowConfigError("pubport_"+i,"<?echo I18N("j", "The input public port is invalid.");?>");
				OBJ("pubport_"+i).focus();
				return null;
			}
			if (OBJ("priport_"+i).value!="" && !TEMP_IsDigit(OBJ("priport_"+i).value))
			{
				BODY.ShowConfigError("priport_"+i,"<?echo I18N("j", "The input private port is invalid.");?>");
				OBJ("priport_"+i).focus();
				return null;
			}
			if (OBJ("ip_"+i).value!="" && !TEMP_CheckNetworkAddr(OBJ("ip_"+i).value, null, null))
			{
				BODY.ShowConfigError("ip_"+i,"<?echo I18N("j", "Invalid host IP address.");?>");
				OBJ("ip_"+i).focus();
				return null;
			}
			if (OBJ("pubport_"+i).value=="" && OBJ("priport_"+i).value=="" && OBJ("dsc_"+i).value!="" && OBJ("pro_"+i).value!="Other")
			{
				BODY.ShowConfigError("pubport_"+i,"<?echo I18N("j", "Invalid Port !.");?>");
				OBJ("pubport_"+i).focus();
				return null;
			}
			if (OBJ("pronum_"+i).value!="" && !TEMP_IsDigit(OBJ("pronum_"+i).value))
			{
				BODY.ShowConfigError("pronum_"+i,"<?echo I18N("h", "The input protocol number is invalid.");?>");
				OBJ("pronum_"+i).focus();
				return null;
			}
			...
			for (var j=1; j < i; j+=1)
			{
				...
				{
					BODY.ShowConfigError("dsc_"+i,"<?echo I18N("j", "The different rules cannot have the same name.");?>");
					OBJ("dsc_"+j).focus();
					return null;
				}	
				if(OBJ("pubport_"+i).value==OBJ("pubport_"+j).value && OBJ("pubport_"+i).value!="") 
				{
					BODY.ShowConfigError("pubport_"+i,"<?echo I18N("j", "The different rules cannot use the same public port.");?>");
				...
				{
					BODY.ShowConfigError("ip_"+i,"<?echo I18N("j", "Rules must use different IP addresses.");?>");
					OBJ("pronum_"+j).focus();
					return null;
				}				
			}			
