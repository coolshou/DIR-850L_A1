...
Page.prototype =
{
	...
	SaveXML: function(wlan_phyinf , wifi_phyinf)
	{
		...
		{
			BODY.ShowMessage("Error","<?echo I18N("j","Invalid beacon interval.");?>");
			OBJ("beacon"+str_Aband).focus();
			return null;
		}
		else if (!TEMP_IsDigit(OBJ("rts"+str_Aband).value))
		{
			BODY.ShowMessage("Error","<?echo I18N("j","Invalid RTS threshold value.");?>");
			OBJ("rts"+str_Aband).focus();
			return null;
		}
		else if (!TEMP_IsDigit(OBJ("frag"+str_Aband).value))
		{
			BODY.ShowMessage("Error","<?echo I18N("j","Invalid fragmentation value");?>");
			OBJ("frag"+str_Aband).focus();
			return null;
		}
		else if (!TEMP_IsDigit(OBJ("dtim"+str_Aband).value))
		{
			BODY.ShowMessage("Error","<?echo I18N("j","Invalid DTIM Interval.");?>");
