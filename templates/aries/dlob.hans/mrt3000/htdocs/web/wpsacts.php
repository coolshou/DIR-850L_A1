...
{
	$result = "FAIL";
	$reason = I18N("j","Permission denied. The user is unauthorized.");
}
else
{
	$i = 0;
	while ($i <= 0)
	{
		...
		if ($_POST["action"] == "PIN")
		{
			...
			if ($pin == 0)	{ fail(I18N("j","Invalid PIN code!"));	break; }

			$path = XNODE_getpathbytarget("/runtime", "phyinf", "uid", $_POST["uid"]);
			if ($path == "") {fail(I18N("j","Invalid Path!")); break;}
		...
		{
			fail(I18N("j","Unknown ACTION!"));	break;
		}
		$RESULT = "OK";
		$REASON = "";
	}
