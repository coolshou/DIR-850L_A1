...
function miiicasasetup($name)
{
	...
	if ($infp=="")
	{
		SHELL_info($_GLOBALS["START"], "miiicasasetup: (".$name.") not exist.");
		SHELL_info($_GLOBALS["STOP"],  "miiicasasetup: (".$name.") not exist.");
		http_error("9");
		return;
	}
	...
	if($af != "inet")
	{
		SHELL_info($_GLOBALS["START"], "miiicasasetup: (".$name.") not ipv4.");
		SHELL_info($_GLOBALS["STOP"],  "miiicasasetup: (".$name.") not ipv4.");
		http_error("9");
		return;
	}
	...
	if ($ifname==""||$af==""||$ipaddr=="")
	{
		SHELL_info($_GLOBALS["START"], "miiicasasetup: (".$name.") no phyinf.");
		SHELL_info($_GLOBALS["STOP"],  "miiicasasetup: (".$name.") no phyinf.");
		http_error("9");
		return;
	}

    $miiicasa = query("/miiicasa/enable");
    $dirty = 0;
    $port = query("/miiicasa/external_port");
	if($port == "")
		$port = 5555;

	$stsp = XNODE_getpathbytarget("/runtime/services/http", "server", "uid", "MIIICASA.".$name, 0);

	if ($stsp=="")
	{
		if ($miiicasa != 1)
		{
			SHELL_info($_GLOBALS["START"], "miiicasasetup: (".$name." with code ".$miiicasa.") not active.");
			SHELL_info($_GLOBALS["STOP"],  "miiicasasetup: (".$name." with code ".$miiicasa.") not active.");
			http_error("8");
			return;
		}
		else
		{
			$dirty++;
			$stsp = XNODE_getpathbytarget("/runtime/services/http","server","uid","MIIICASA.".$name,1);
			set($stsp."/mode",  "MIIICASA");
			set($stsp."/inf",   $name);
	...
	{
		if ($miiicasa != 1) { $dirty++; del($stsp); }
		else
		{
			if (query($stsp."/mode")!="MIIICASA")   { $dirty++; set($stsp."/mode", "MIIICASA"); }
	...
	stopcmd('sh /etc/scripts/delpathbytarget.sh runtime/services/http server uid MIIICASA.'.$name);
...
function httpsetup($name)
{
	...
	else
	{
		...
		else
		{
			...
			if (query($stsp."/af")!=$af)			{ $dirty++; set($stsp."/af", $af); }
			if (query($stsp."/miiicasa")!=$miiicasa)			{ $dirty++; set($stsp."/miiicasa", $miiicasa); }
		}
	}
	...
	$vender = query("/runtime/device/vendor");
	$srvname = $vender." ".$product;
	...
	{
		$mdirty = setup_mdns("MDNSRESPONDER.WFA", "8181", $vender." SharePort Web Access", "_http._tcp. local.");
	}

	/* info for QRS Mobile */
	if($hnap > 0)
	{
		$mdirty = setup_mdns("MDNSRESPONDER.HNAP", "80", $vender." HNAP Service", "_dhnap._tcp. local.");
	}	
