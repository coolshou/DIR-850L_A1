...
if($sharepath=="" && $active =="1")
{ 
	/* set the first "supported partition" as DLNA's sharepath if it's empty, Sammy */
	foreach("/runtime/device/storage/disk")
	{
		foreach("entry")
		{
			...
			{
				$storage_mntp = query("mntp");
				$sharepath = cut($storage_mntp, 4, "/");
				break;
			}
		}
	}
...
else
{
	...
	{														
		...
		//fwrite("a", $START, "upnp 0 \"".$vendor." Corporation\" \"".$product_url."\" \"Network File Server with USB2.0 and wifi interface\" ". "\"".$Genericname."\" \"".$product_url."\" ".$model_name." ".$mac_addr." ".$dms_name." &\n");
