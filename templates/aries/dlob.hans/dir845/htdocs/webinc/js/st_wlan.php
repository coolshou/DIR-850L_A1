...
Page.prototype =
{	
	services: "WIFI.PHYINF,RUNTIME.INF.LAN-1,RUNTIME.INF.LAN-2,RUNTIME.PHYINF,RUNTIME.ANTENNA",
	...
	InitValue: function(xml)
	{
		...
		PAGE.FillTable("BAND24G-1.2", "WIFI.PHYINF", "RUNTIME.PHYINF", "RUNTIME.ANTENNA");
		}
		PAGE.FillTable("BAND24G-1.1", "WIFI.PHYINF", "RUNTIME.PHYINF", "RUNTIME.ANTENNA");
		
		this.dual_band = COMM_ToBOOL('<?=$FEATURE_DUAL_BAND?>');
		if(this.dual_band)
		{
			OBJ("div_5G").style.display = "block";
			PAGE.FillTable("BAND5G-1.1", "WIFI.PHYINF", "RUNTIME.PHYINF", "RUNTIME.ANTENNA");
			PAGE.FillTable("BAND5G-1.2", "WIFI.PHYINF", "RUNTIME.PHYINF", "RUNTIME.ANTENNA");
		}
	},	
	
	FillTable : function (wlan_uid, wifi_phyinf ,runtime_phyinf ,runtime_antenna)
	{		
		var wifi_module 	= PXML.FindModule(wifi_phyinf);
		var rwifi_module 	= PXML.FindModule(runtime_phyinf);
		var antenna_module 	= PXML.FindModule(runtime_antenna);
		...
		for (var i=1; i<=cnt; i++)
		{
			...
			var antenna_p = GPBT(antenna_module+"/runtime/antenna","entry","macaddr",mac, false);
			var antenna	= XG(antenna_p+"/antenna");
			//The labels of the smart antenna E2 and E5 are upside down. Modify the labels in the GUI.
			/*if(!(antenna.match("E2")=="E2" && antenna.match("E5")=="E5"))
			{
				if(antenna.match("E2")=="E2") antenna = antenna.replace("E2","E5");
				else if(antenna.match("E5")=="E5") antenna = antenna.replace("E5","E2");
			}*/
			var data	= [mac, ipaddr, mode, rate, rssi, antenna];
