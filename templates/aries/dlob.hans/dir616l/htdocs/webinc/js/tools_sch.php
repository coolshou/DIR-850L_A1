...
Time.prototype =
{
	...
	To12Hrs: function()
	{
		if(this.apm !== "" && this.hour >= 1 && this.hour <= 12) return;
	...
	To24Hrs: function()
	{
		if ((this.apm === "" && this.hour >= 0 && this.hour <= 23) ||
		    (this.apm !== "" && (this.hour <= 0 || this.hour > 12))) 
			return;
		
		if (this.hour == 12 && this.apm === "AM"){ this.hour=0;}
		if (this.hour !== 12 && this.apm === "PM"){ this.hour+=12;}
...
Page.prototype =
{
	...
	OnClick24Hours: function()
	{
		...
		if (checked)
		{
			OBJ("schstarthrs").value = "00"; 
			OBJ("schstartmin").value = "00";
			COMM_SetSelectValue(OBJ("schstartapm"), "AM");
			OBJ("schendhrs").value = "23";
	...
	{
		var Start = new Time(OBJ("schstarthrs").value);
		var End = new Time(OBJ("schendhrs").value);
		...
		OBJ("schstarthrs").value = Start.hour;
		OBJ("schendhrs").value = End.hour;
	},
