...
Page.prototype =
{
	...
	Initial: function(wlan_phyinf,wifi_phyinf)
	{
		...
		if(freq == "5")
			...
			str_Aband = "";
		var en_wifi = COMM_ToBOOL(XG(this.phyinf+"/active"));
		if (en_wifi == "1")
		{	
			...
			if(COMM_ToBOOL(XG(this.phyinf+"/media/dot11n/bw2040coexist"))== true) 	
				OBJ("coexist_enable"+str_Aband).checked = true;
			else 	
				OBJ("coexist_disable"+str_Aband).checked = true;			
			...
			if (!this.rgmode)
			{
				//Not in rgmode mode, should be in WDS mode, gray coexist options.
				OBJ("coexist_enable").disabled = true;
				OBJ("coexist_disable").disabled = true;
			}
		...
		{
			OBJ("tx_power"+str_Aband).disabled = true;
                        OBJ("wlan_partition"+str_Aband).disabled = true;
			OBJ("en_wmm"+str_Aband).disabled = true;
 			OBJ("coexist_enable").disabled = true;
                   	OBJ("coexist_disable").disabled = true;
		}
