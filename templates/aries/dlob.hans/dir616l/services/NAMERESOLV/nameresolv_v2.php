	...
	foreach ("/runtime/services/nameresolv/ifname")
	{
		...
		while ($i < $cnt)
		{
			...
			{ 
				...
				{
					/* if ipv4 ipalias exists, enable netbios,too */
					$p = XNODE_getpathbytarget("/runtime", "inf", "uid", "BRIDGE-1", 0);
					if ($p != "")
					{
						$aip = query($p."/ipalias/ipv4/ipaddr:1");
						if ($aip != "") $netbios_enable=1;
					}
					$llmnresp_enable=1;
					$active++;
				}	
		...
		if ($active > 0 )
		{
			...
			{
				$param = $param." -i ".$ifname." -r ".$ahostname;
			}
			else
			{
