...
function apcli_parameters($uid)
{
	...
	if (query("opmode") == "APCLI" && query($p."/active") == "1")
	{
		$ssid = query("ssid");
		$auth = query("authtype");

		if 		($auth == "SHARED")		$authtype = "SHARED";
		else if ($auth == "WEPAUTO")	$authtype = "WEPAUTO";
		...
		/* encrtype */
		$encrypt = query("encrtype");
		if ($encrypt == "TKIP+AES")		$encrtype = "TKIPAES";
		else							$encrtype = $encrypt;

		echo "ApCliEnable=1\n";
		echo "ApCliSsid=".		$ssid. "\n";
		echo "ApCliAuthMode=".	$authtype	."\n";
		echo "ApCliEncrypType=".$encrtype	."\n";

		if ($encrypt == "WEP")
		{
			$defkeyid = query("nwkey/wep/defkey");
			echo "ApCliDefaultKeyID=".	$defkeyid."\n";
			echo "ApCliKey".$defkeyid."Type=".	query("nwkey/wep/ascii")."\n";
			echo "ApCliKey".$defkeyid."Str=".query("nwkey/wep/key:".$defkeyid)."\n";
		} else if ($encrypt == "TKIP" || $encrypt == "AES" || $encrypt == "TKIP+AES")
		{
			echo "ApCliWPAPSK=".query("nwkey/psk/key")."\n";
		}
