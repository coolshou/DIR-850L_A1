...
static void usage(int status)
{
	...
	fprintf(stream, "usage: %s [-h] [-e edition] [-i file] [-n name] dirname outfile\n"
		...
		" -l         import symlink contents when linkname start with \"@\"\n"
		" -r         rational cramfs, set all owner and group to root:root\n"
...
static int cramsort (const void *a, const void *b)
{
        ...
        const char *p = (atlink_mode && (*A)->d_name [0] == '@' && (*A)->d_name [1])
                ? (((*A)->d_name) + 1) : ((*A)->d_name);
        const struct dirent **B = (const struct dirent **) b;
        const char *q = (atlink_mode && (*B)->d_name [0] == '@' && (*B)->d_name [1])
                ? (((*B)->d_name) + 1) : ((*B)->d_name);
        return strcmp (p, q);
}
...
static unsigned int parse_directory(struct entry *root_entry, const char *name, struct entry **prev, loff_t *fslen_ub)
{
	...
	int totalsize = 0, dircount, dirindex, truncAt;
	char *path, *endpath;
	...
	*endpath = '/';
	...
	for (dirindex = 0; dirindex < dircount; dirindex++) {
		...
		if (lstat(path, &st) < 0) {
        	...
        	if (atlink_mode && dirent->d_name[0] == '@' && dirent->d_name[1]) {
                    truncAt = 1;
                    namelen--;
		...
		if (!entry) {
			die(MKFS_ERROR, 1, "calloc failed");
                ...
                if (truncAt)
                    entry->name = strdup((dirent->d_name) + 1);
                else
		...
		entry->uid = st.st_uid;
#else
		entry->uid = (root_mode) ? 0 : st.st_uid;
#endif
		...
		entry->gid = st.st_gid;
#else
		entry->gid = (root_mode) ? 0 : st.st_gid;
#endif
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "lrhEe:i:n:psvz")) != EOF) {
		switch (c) {
		case 'h':
			usage(MKFS_OK);
		case 'E':
...
#else
	root_entry->uid = (root_mode) ? 0 : st.st_uid;
        root_entry->gid = (root_mode) ? 0 : st.st_gid;
#endif
